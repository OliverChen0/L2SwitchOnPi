...
)
{
    ...
    for ( port = switch_range->begin_port; port < end_port; ++port ) {
        ...
        if ( mrdc_icli_port_get(&spr) == FALSE ) {
    ...
    for ( pt = ICLI_PORT_TYPE_FAST_ETHERNET; pt < ICLI_PORT_TYPE_MAX; ++pt ) {
        ...
        for ( sid = 1; sid < ICLI_MAX_SWITCH_ID; ++sid ) {
            ...
            for ( port = 1; port < ICLI_MAX_PORT_ID; ++port ) {
                ...
                if ( mrdc_icli_port_get(&spr) && _switch_range_result_add(&spr, result) ) {
                    ++valid_cnt;
                }
            } // for port
        } // for sid
...
)
{
    ...
    // device name
    (void)mrdc_icli_str_ncpy(conf->dev_name, ICLI_DEFAULT_DEVICE_NAME, ICLI_DEV_NAME_MAX_LEN);

    // mode name
    for ( i = 0; i < ICLI_CMD_MODE_MAX; ++i ) {
        (void)mrdc_icli_str_ncpy(conf->mode_prompt[i], icli_platform_mode_prompt_get((icli_cmd_mode_t)i), ICLI_MODE_PROMPT_MAX_LEN);
    }

    // reset session config data to default
    for ( i = 0; i < ICLI_SESSION_CNT; ++i ) {
        (void)mrdc_icli_session_config_data_default( &(conf->session_config[i]) );
    }
}
#endif
...
)
{
    ...
    /* init parsing */
    if ( mrdc_icli_parsing_init() != ICLI_RC_OK ) {
        T_E("fail to init parsing\n");
        return ICLI_RC_ERROR;
    }

    /* init register */
    if ( mrdc_icli_register_init() != ICLI_RC_OK ) {
        T_E("fail to init register\n");
        return ICLI_RC_ERROR;
    }

    /* init session */
    if ( mrdc_icli_session_init() != ICLI_RC_OK ) {
        T_E("fail to init session\n");
        return ICLI_RC_ERROR;
    }

    /* init execution */
    if ( mrdc_icli_exec_init() != ICLI_RC_OK ) {
        T_E("fail to init execution\n");
        return ICLI_RC_ERROR;
    }

    /* init priv */
    if ( mrdc_icli_priv_init() != ICLI_RC_OK ) {
...
/*
    ...
    COMMENT
        n/a
*/
icli_conf_data_t *mrdc_icli_conf_get(
    void
)
{
    return &g_conf_data;
...
)
{
    ...
    // apply to runtime
    for ( i = 0; i < ICLI_SESSION_CNT; ++i ) {
        handle = mrdc_icli_session_handle_get(i);
...
/*
    ...
    comment -
        n/a
*/
icli_input_style_t mrdc_icli_input_style_get(void)
{
...
/*
    ...
    comment -
        n/a
*/
u32 mrdc_icli_case_sensitive_get(void)
{
...
/*
    ...
    comment -
        n/a
*/
u32 mrdc_icli_console_alive_get(void)
{
...
/*
    ...
    COMMENT
        n/a
*/
char *mrdc_icli_mode_prompt_get(
...
)
{
    ...
    (void)mrdc_icli_str_ncpy(g_conf_data.mode_prompt[mode], prompt, ICLI_MODE_PROMPT_MAX_LEN);
...
/*
    ...
    COMMENT
        n/a
*/
char *mrdc_icli_dev_name_get(void)
{
...
)
{
    ...
    (void)mrdc_icli_str_ncpy(g_conf_data.dev_name, dev_name, ICLI_DEV_NAME_MAX_LEN);
...
/*
    ...
    COMMENT
        n/a
*/
char *mrdc_icli_banner_login_get(void)
{
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_banner_login_set(
...
)
{
    if ( banner_login == NULL ) {
        ...
        (void)mrdc_icli_str_ncpy(g_conf_data.banner_login, banner_login, ICLI_BANNER_MAX_LEN);
...
/*
    ...
    COMMENT
        n/a
*/
char *mrdc_icli_banner_motd_get(void)
{
...
)
{
    if ( banner_motd == NULL ) {
        ...
        (void)mrdc_icli_str_ncpy(g_conf_data.banner_motd, banner_motd, ICLI_BANNER_MAX_LEN);
...
/*
    ...
    COMMENT
        n/a
*/
char *mrdc_icli_banner_exec_get(void)
{
...
)
{
    if ( banner_exec == NULL ) {
        ...
        (void)mrdc_icli_str_ncpy(g_conf_data.banner_exec, banner_exec, ICLI_BANNER_MAX_LEN);
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_type_present(
    IN icli_port_type_t     port_type
)
{
    u32     i;
...
/*
    ...
    COMMENT
        n/a
*/
icli_port_type_t mrdc_icli_port_next_present_type(
    IN icli_port_type_t     port_type
)
{
    /* get next type */
...
/*
    ...
    *MUST*  after using, mrdc_icli_port_range_restore() is called to restore the
    ...
    COMMENT
        n/a
*/
void mrdc_icli_port_range_backup( void )
{
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_port_range_restore( void )
{
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_port_range_reset( void )
{
...
#ifndef MRDC_PRODUCT_RACOON // RACOON设备的端口从0开始，则此处不检查
        /* check port ID */
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_from_usid_uport(
...
)
{
    ...
    for ( i = 0; i < g_port_range.cnt; ++i ) {
        ...
        /* in range, get data */
        T_I("index: %d, g_begin_port: %d, begin_uport: %d, g_begin_uport: %d", i,
            g_port_range.switch_range[i].begin_port, switch_range->begin_uport,
            g_port_range.switch_range[i].begin_uport);
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_from_isid_iport(
    INOUT icli_switch_port_range_t  *switch_range
)
{
    u32     i;
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_switch_port_valid(
    IN u16      port_type,
    IN u16      switch_id,
    IN u16      port_id
)
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_switch_range_valid(
...
)
{
    ...
    for ( port = switch_port->begin_port; port <= end_port; ++port ) {
        if ( mrdc_icli_port_switch_port_valid(switch_port->port_type, switch_port->switch_id, port) == FALSE ) {
            if ( switch_id && port_id ) {
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_stack_range_valid(
...
)
{
    ...
    for ( i = 0; i < stack_port->cnt; ++i ) {
        if ( mrdc_icli_port_switch_range_valid(&(stack_port->switch_range[i]), switch_id, port_id) == FALSE ) {
            return FALSE;
        }
    }
    return TRUE;
...
*/
BOOL mrdc_icli_port_type_list_get(
    IN    icli_port_type_t          port_type,
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_port_type_list_add(
    IN  icli_stack_port_range_t   *src,
    OUT icli_stack_port_range_t   *dst
)
{
...
)
{
    ...
    if ( mrdc_icli_port_get_first(&spr) ) {
        if ( spr.switch_id == switch_range->switch_id ) {
            b = TRUE;
        } else {
            while ( mrdc_icli_port_get_next(&spr) ) {
                if ( spr.switch_id == switch_range->switch_id ) {
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_switch_get_next(
    INOUT icli_switch_port_range_t  *switch_range
)
{
    icli_switch_port_range_t    spr;
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_switch_port_get(
    INOUT icli_switch_port_range_t  *switch_range
)
{
    return mrdc_icli_port_get( switch_range );
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_switch_port_get_next(
    INOUT icli_switch_port_range_t  *switch_range
)
{
    icli_switch_port_range_t    spr;
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_enable_password_get(
...
)
{
    ...
    (void)mrdc_icli_str_cpy(password, g_conf_data.enable_password[priv]);
    return TRUE;
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_enable_password_set(
    IN  icli_privilege_t    priv,
...
)
{
    ...
    if ( mrdc_icli_str_len(password) > ICLI_PASSWORD_MAX_LEN ) {
        T_E("the length of password is too long\n");
        return FALSE;
    }

    g_conf_data.b_enable_secret[priv] = FALSE;
    (void)mrdc_icli_str_cpy(g_conf_data.enable_password[priv], password);
    return TRUE;
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_enable_password_verify(
    IN  icli_privilege_t    priv,
...
)
{
    ...
    if ( g_conf_data.b_enable_secret[priv] ) {
        // get mac
        icli_platform_hmac_md5(g_enable_key, g_enable_key_len, (u8 *)clear_password, mrdc_icli_str_len(clear_password), mac);

        // translate into string
        _mac_to_string(mac, str);

        if ( mrdc_icli_str_cmp(str, g_conf_data.enable_password[priv] ) == 0 ) {
            return TRUE;
        } else {
            return FALSE;
        }
    } else {
        if ( mrdc_icli_str_cmp(clear_password, g_conf_data.enable_password[priv] ) == 0 ) {
            return TRUE;
        } else {
            return FALSE;
        }
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_enable_secret_set(
    IN  icli_privilege_t    priv,
...
)
{
    ...
    if ( mrdc_icli_str_len(secret) > ICLI_PASSWORD_MAX_LEN ) {
        T_E("the length of secret is too long\n");
        return FALSE;
    }

    g_conf_data.b_enable_secret[priv] = TRUE;
    (void)mrdc_icli_str_cpy(g_conf_data.enable_password[priv], secret);
    return TRUE;
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_enable_secret_set_clear(
    IN  icli_privilege_t    priv,
    IN  char                *clear_password
)
{
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_enable_password_if_secret_get(
    IN  icli_privilege_t    priv
)
{
