...
==============================================================================
*/
#define ICLI_EXEC_PUT_SPACE     mrdc_icli_session_char_put(handle, ICLI_SPACE)
#define ICLI_EXEC_PUT_NEWLINE   mrdc_icli_session_char_put(handle, ICLI_NEWLINE)
...
)
{
    switch ( rc ) {
    case ICLI_RC_OK:
        if ( mrdc_icli_variable_data_string_type_get(type) ) {
            return ICLI_MATCH_TYPE_PARTIAL;
        } else {
#if 1 /* CP, Bugzilla#14843 - ambiguous between <uint> and <port_type> */
            switch (type) {
            case ICLI_VARIABLE_PORT_TYPE:
                if ( mrdc_icli_str_sub(w, mrdc_icli_variable_port_type_get_name(value->u.u_port_type), FALSE, NULL) != 0 ) {
                    return ICLI_MATCH_TYPE_PARTIAL;
                }
...
)
{
    ...
    if ( mrdc_icli_parsing_random_head(must_head, node, NULL) ) {
        return TRUE;
    }
    ...
    for ( child = node->child; child != NULL; ___CHILD(child) ) {
        if ( mrdc_icli_parsing_random_head(must_head, child, NULL) ) {
            return TRUE;
        }
...
)
{
    ...
    if ( runtime_cb ) {
        b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_PRESENT, &(handle->runtime_data.runtime) );
...
)
{
    ...
    */
    if ( mrdc_icli_session_debug_cmd_allow_get(handle) == FALSE ) {
        if ( node->parent_cnt == 0 && node->type == ICLI_VARIABLE_KEYWORD ) {
            if ( mrdc_icli_str_sub(node->word, "debug", 0, NULL) == 0 ) {
                return FALSE;
            }
        }
    }
...
)
{
    ...
    (void)mrdc_icli_session_error_printf( handle, "%% Valid ports: ");

    for ( i = 0; i < spr->cnt; i++ ) {
        if ( i ) {
            (void)mrdc_icli_session_error_printf( handle, ", " );
        }
        (void)mrdc_icli_session_error_printf( handle, "%s ", mrdc_icli_variable_port_type_get_short_name(spr->switch_range[i].port_type) );
        (void)mrdc_icli_session_error_printf( handle, "%s ", _port_list_str_get(spr, i, str) );
    }
    (void)mrdc_icli_session_error_printf( handle, "\n\n");
}
#endif
...
)
{
    ...
    p = mrdc_icli_variable_url_protocol_get(w);
...
)
{
    ...
    if ( *para ) {
        ...
        p = mrdc_icli_exec_parameter_get();
        if ( p == NULL ) {
    ...
    case ICLI_VARIABLE_KEYWORD :
        ...
        if ( *w ) {
            ...
            /* get the result */
            rc = mrdc_icli_str_sub(w, node->word, mrdc_icli_case_sensitive_get(), NULL);
            /* post-process w */
            if ( c ) {
                // put space back
                *c = ICLI_SPACE;
        ...
        case 1 : /* partial match */
            ...
            if ( *w ) {
                ...
                (void)mrdc_icli_str_ncpy(p->value.u.u_word, w, ICLI_VALUE_STR_MAX_LEN);
            } else {
    ...
    case ICLI_VARIABLE_CWORD:
        if ( mrdc_icli_str_len(w) == 0 ) {
            ...
            k = mrdc_icli_exec_cword_match_cnt( handle, w, runtime_cb, &(handle->runtime_data.runtime), &match_type, &cword );
            if ( k >= 1 ) {
                p->value.type = ICLI_VARIABLE_KEYWORD;
                (void)mrdc_icli_str_ncpy(p->value.u.u_cword, (k == 1) ? cword : w, ICLI_VALUE_STR_MAX_LEN);
                break;
            } else if ( k == 0 ) {
                break;
            } else if ( k < 0 ) {
    ...
    default :
        ...
        if ( mrdc_icli_str_len(w) == 0 ) {
            ...
            case ICLI_VARIABLE_VCAP_VR:
                ...
                b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_VCAP_VR, &(handle->runtime_data.runtime) );
                if ( b ) {
            ...
            case ICLI_VARIABLE_NET_ADDR:
                if (runtime_cb) {
                    b = mrdc_icli_exec_runtime(handle, runtime_cb, ICLI_ASK_NET_ADDR, &(handle->runtime_data.runtime));
                    if (b) {
                        n = 0;
                        range = &(handle->runtime_data.runtime.range);
            ...
            rc = mrdc_icli_variable_get(type, w, range, &(p->value), &err_pos);

#if 1 /* the output of mrdc_icli_variable_get() is different from the origianl type, so it need translation */
            switch ( type ) {
                ...
                case ICLI_VARIABLE_PORT_TYPE:
                    ...
                    case ICLI_VARIABLE_PORT_TYPE_ID:
                        ...
                        if ( mrdc_icli_port_get(&spr) ) {
                            ...
                            (void)mrdc_icli_session_error_printf( handle, "%% No such interface: %s %u/%u\n",
                                                                  mrdc_icli_variable_port_type_get_name(port_type),
                    ...
                    case ICLI_VARIABLE_PORT_TYPE_LIST:
                        ...
                        default:
                            switch_id = 0;
                            if ( mrdc_icli_port_type_list_get( port_type, &(p->value.u.u_port_list),
                                                               &switch_id, &port_id ) ) {
                                ...
                                if ( switch_id == ICLI_SWITCH_PORT_ALL ) {
                                    ...
                                    (void)mrdc_icli_session_error_printf( handle, "%% No valid port in wildcard, %s %s\n",
                                                                          mrdc_icli_variable_port_type_get_name(port_type),
                    ...
                    case ICLI_VARIABLE_SWITCH_ID:
                        ...
                        if ( mrdc_icli_switch_get(&spr) == FALSE ) {
                            rc = ICLI_RC_ERR_MATCH;
                            memset(handle->runtime_data.cmd_copy, 0, sizeof(handle->runtime_data.cmd_copy));
                            (void)mrdc_icli_session_error_printf(handle, "%% No such switch ID: %u\n",
                                                                 spr.switch_id);
                        }
                        break;
                            ...
                            for ( j = pur->range[i].min; j <= pur->range[i].max; ++j ) {
                                ...
                                if ( mrdc_icli_switch_get(&spr) == FALSE ) {
                                    if ( b_first ) {
                                        ...
                                        rc = ICLI_RC_ERR_MATCH;
                                        memset(handle->runtime_data.cmd_copy, 0, sizeof(handle->runtime_data.cmd_copy));
                                        (void)mrdc_icli_session_error_printf(handle, "%% No such switch ID: %u", spr.switch_id);
                                    } else {
                                        (void)mrdc_icli_session_error_printf(handle, ",%u", spr.switch_id);
                                    }
                                }
                            }
                        }
            ...
            case ICLI_RC_ERR_INCOMPLETE:
...
#endif
                    if ( mrdc_icli_exec_port_type_present(handle, node, p->value.u.u_port_type) == FALSE ) {
                        err_pos = 0;
                        rc = ICLI_RC_ERR_MATCH;
                    }
                    break;
                ...
                case ICLI_VARIABLE_RANGE_URL_FILE:
                    ...
                    b = FALSE;
                    if (runtime_cb) {
                        b = mrdc_icli_exec_runtime(handle, runtime_cb, ICLI_ASK_URL, &(handle->runtime_data.runtime));
                        if (b && _url_runtime_chk(&(handle->runtime_data.runtime), w) == FALSE) {
                            rc = ICLI_RC_ERR_MATCH;
                            (void)mrdc_icli_session_error_printf(handle, "%% Protocol not supported.\n");
                        }
                    }
                    break;
                    ...
                    if (runtime_cb) {
                        ...
                        if (b && handle->runtime_data.runtime.net_addr_not_support[p->value.u.u_net_addr.type]) {
                            ...
                            case ICLI_NET_ADDR_TYPE_IPV4:
                                (void)mrdc_icli_session_error_printf(handle, "%% IPv4 not supported.\n");
                                break;

                            case ICLI_NET_ADDR_TYPE_IPV6:
                                (void)mrdc_icli_session_error_printf(handle, "%% IPv6 not supported.\n");
                                break;

                            case ICLI_NET_ADDR_TYPE_DOMAIN_NAME:
                                (void)mrdc_icli_session_error_printf(handle, "%% Domain name not supported.\n");
                                break;
        ...
        case ICLI_MATCH_TYPE_PARTIAL : /* partial match */
            ...
            if ( handle->runtime_data.match_para ) {
                ...
                if ( handle->runtime_data.b_in_loop == FALSE ) {
                    ...
                    */
                    if ( b == TRUE ) {
                        mrdc_icli_exec_parameter_free( para );
                        break;
                    }
#if 1 /* CP, 2012/09/07 11:52, <port_type_list> */
                }
...
)
{
    ...
    if ( handle->runtime_data.b_exec_by_api ) {
        if ( handle->runtime_data.app_err_msg ) {
            ...
            if ( handle->runtime_data.line_number == ICLI_INVALID_LINE_NUMBER ) {
                mrdc_icli_session_str_put(handle, handle->runtime_data.app_err_msg);
                ICLI_EXEC_PUT_NEWLINE;
            } else {
                mrdc_icli_session_str_printf(handle, "%% Error in file %s, line %d:\n",
    ...
    if ( w == NULL ) {
        ...
        mrdc_icli_session_str_put(handle, err_descr);
        ICLI_EXEC_PUT_NEWLINE;
        return;
    }
    ...
    if ( handle->runtime_data.more_print ) {
        mrdc_icli_session_str_put(handle, handle->runtime_data.cmd_copy);
    ...
    if ( (handle->runtime_data.cmd_len > _CMD_LINE_LEN) ||
        ...
        for ( i = 0; i < (u32)(handle->runtime_data.cmd_len); ++i ) {
            mrdc_icli_session_char_put(handle, handle->runtime_data.cmd[i]);
            if ( (i + 1) % (handle->runtime_data.width - 1) == 0 ) {
                ...
                if ( w_pos < i ) {
                    ...
                    mrdc_icli_session_char_put(handle, '^');
                    ICLI_EXEC_PUT_NEWLINE;
    ...
    mrdc_icli_session_str_printf(handle, "%% %s word detected at '^' marker.\n", err_descr);
...
)
{
    ...
    if ( mrdc_icli_str_cmp(para->match_node->word, p->match_node->word) == -1 ) {
        para->next = p;
        handle->runtime_data.match_sort_list = para;
    } else {
        for ( prev = p, ___NEXT(p);
              (p) && (mrdc_icli_str_cmp(para->match_node->word, p->match_node->word) == 1);
              prev = p, ___NEXT(p) ) {
            ;
        }
        // insert
...
)
{
    ...
    if ( handle->runtime_data.cmd_var ) {
        ...
        if ( p->match_node->type == ICLI_VARIABLE_KEYWORD && mrdc_icli_str_sub(p->match_node->word, "interface", 0, NULL) == 0 ) {
...
#if 1 /* Bugzilla#12915 - invalud hidden interface entry command, then back to global-config mode */
...
)
{
    ...
    // empty command
    if ( mrdc_icli_str_len(handle->runtime_data.cmd_walk) == 0 ) {
        return ICLI_RC_ERR_EMPTY;
    }

    // get tree
    if ( handle->runtime_data.b_in_parsing ) {
        tree = mrdc_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.current_parsing_mode) );
    } else {
        tree = mrdc_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.mode_para[handle->runtime_data.mode_level].mode) );
    }
...
_CMD_WALK_REDO:
        ...
        default : /* multiple match, then find the exact match */
            ...
            if ( handle->runtime_data.exactly_cnt == 0 ) {
                for ( p = handle->runtime_data.match_para; p; ___NEXT(p) ) {
                    if ( mrdc_icli_variable_data_string_type_get((icli_variable_type_t)(p->value.type)) ) {
                        p->match_type = ICLI_MATCH_TYPE_EXACTLY;
                        ++( handle->runtime_data.exactly_cnt );
                    }
                }
        ...
        for ( p = handle->runtime_data.cmd_var; p; ___NEXT(p) ) {
            if ( b ) {
                ...
                case ICLI_VARIABLE_PORT_TYPE_LIST:
                    ...
                    if ( mrdc_icli_port_type_list_add(&(p->value.u.u_port_type_list), para_spr) == FALSE ) {
                        T_E("mrdc_icli_port_type_list_add()\n");
                        break;
                    }

                    // free prev
                    mrdc_icli_exec_parameter_free( prev );
...
)
{
    ...
    *c = ICLI_EOS;
    ++c;
    (void)mrdc_icli_str_cpy(c, handle->runtime_data.cmd);
    handle->runtime_data.cmd_walk = c;
...
)
{
    ...
    */
                ...
                if ( d == -1 ) {
                    if ( b_add ) {
                        if ( para->match_node != handle->runtime_data.random_must_head[i] ) {
                            if ( mrdc_icli_parsing_random_head(handle->runtime_data.random_must_head[i], para->match_node, NULL) == FALSE ) {
                                return;
                            }
                        }
                    } else {
    ...
    case ICLI_VARIABLE_PORT_LIST:
        if ( prev_para && prev_para->value.type == ICLI_VARIABLE_PORT_TYPE ) {
            if ( mrdc_icli_port_type_list_get( prev_para->value.u.u_port_type,
...
)
{
    ...
    if ( _session_random_must_get(handle) == FALSE ) {
        ...
        _handle_para_free( handle );
        if ( last_node ) {
            mrdc_icli_exec_parameter_free( last_node );
        }
        return ICLI_RC_ERROR;
    }
#endif
...
)
{
    ...
    for ( cmd_var = handle->runtime_data.cmd_var; cmd_var; ___NEXT(cmd_var) ) {
        if ( cmd_var->value.type == ICLI_VARIABLE_PORT_TYPE                          &&
            ...
            */
            q = mrdc_icli_exec_parameter_get();
            if ( q == NULL ) {
                T_E("memory insufficient\n");
                // free all parameters
                _handle_para_free( handle );
            ...
            if ( mrdc_icli_exec_port_range_get(handle, q->match_node, &(q->value.u.u_port_type_list)) == FALSE ) {
                T_E("get port range\n");
            }
            ...
            if ( node_prop->cmd_property->cmd_id == p->match_node->execution->cmd_id ) {
                if ( node_prop->cmd_property->mode_destroy != ICLI_CMD_MODE_MAX ) {
                    ...
                    for ( session_id = 0; session_id < ICLI_SESSION_CNT; ++session_id ) {
                        ...
                        if ( mrdc_icli_session_alive(session_id) == FALSE ) {
                            // skip not-alive session
                            continue;
                        }

                        // get current mode
                        rc = mrdc_icli_session_mode_get( session_id, &cmd_mode );
                        if ( rc != ICLI_RC_OK ) {
                        ...
                        if ( (u32)cmd_mode == node_prop->cmd_property->mode_destroy ) {
                            ...
                            session_h = mrdc_icli_session_handle_get( session_id );
                            if ( session_h == NULL ) {
                            ...
                            if ( mrdc_icli_str_cmp(mode_var->value.u.u_range_word, cmd_var->value.u.u_range_word) == 0 ) {
                                ...
                                handle->runtime_data.err_display_mode = ICLI_ERR_DISPLAY_MODE_PRINT;
                                mrdc_icli_session_str_printf(handle, "%% Cannot delete %s; it's being configured in another session. Please try again later.\n", str);
                                handle->runtime_data.err_display_mode = orig_err_display_mode;

                                b_concurrent = TRUE;
                                break;
                            } // if ( mrdc_icli_str_cmp() )
...
/*
    ...
    TOTALLY the same with _session_printf() in mrdc_icli_session.c
...
)
{
    ...
    for ( i = 0; i < ICLI_CMD_CNT; ++i ) {
        if ( g_full_cmd_list[i] ) {
            r = mrdc_icli_str_cmp( g_full_cmd_list[i], cmd );
            switch ( r ) {
            case 1:
                /* place to insert */
...
)
{
    ...
    if ( mrdc_icli_str_len(handle->runtime_data.cmd_walk) == 0 ) {
        // empty then get first possible node
        if ( handle->runtime_data.b_in_parsing ) {
            node = mrdc_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.current_parsing_mode) );
        } else {
            node = mrdc_icli_parsing_tree_get( (icli_cmd_mode_t)(handle->runtime_data.mode_para[handle->runtime_data.mode_level].mode) );
        }
        b_next = TRUE;
    } else {
        // execute command
        ...
        if ( _present_chk(handle, node) && _privilege_chk(handle, node) && _visible_chk(handle, node) ) {
            for ( node_prop = &(node->node_property); node_prop; ___NEXT(node_prop) ) {
                if ( _node_prop_present(handle, node_prop)                                  &&
                    ...
                    if ( n < ICLI_CMD_CNT ) {
                        if ( _sort_insert(mrdc_icli_register_var_cmd_get(node_prop->cmd_property->cmd_id)) ) {
                            ++n;
                        }
                    } else {
        ...
        for ( node_prop = &(node->node_property); node_prop; ___NEXT(node_prop) ) {
            if ( _node_prop_present(handle, node_prop)                                  &&
                ...
                if ( n < ICLI_CMD_CNT ) {
                    ...
                    _sort_insert( mrdc_icli_register_var_cmd_get(node_prop->cmd_property->cmd_id) );
#else
                    _sort_insert( mrdc_icli_register_cmd_str_get(node_prop->cmd_property->cmd_id) );
#endif
                    ++n;
                } else {
                    T_E("command count overflow\n");
...
/*
    ...
    COMMENT
        n/a
*/
icli_parameter_t *mrdc_icli_exec_parameter_get(void)
{
    icli_parameter_t    *p;
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_exec_parameter_free(
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_exec_para_list_free(
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_exec_port_type_present(
    IN icli_session_handle_t    *handle,
    IN icli_parsing_node_t      *node,
...
)
{
    ...
    if ( b ) {
        ...
        /* set system port range for temp use */
        b = FALSE;
        if ( mrdc_icli_port_range_set( &(handle->runtime_data.runtime.port_range) ) ) {
            b = mrdc_icli_port_type_present( port_type );
        } else {
            T_E("Fail to set runtime port range\n");
        }
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_exec_port_range_get(
    IN  icli_session_handle_t    *handle,
    IN  icli_parsing_node_t      *node,
...
)
{
    ...
    if ( b ) {
        ...
        b = FALSE;
        if ( mrdc_icli_port_range_set( &(handle->runtime_data.runtime.port_range) ) ) {
            b = mrdc_icli_port_range_get( port_range );
        } else {
            T_E("Fail to set runtime port range\n");
        }
...
void mrdc_icli_exec_cword_runtime_sort(
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        if ( irt.cword[0] ) {
            for ( j = 0; j < ICLI_CWORD_MAX_CNT; ++j ) {
                ...
                r = mrdc_icli_str_cmp(irt.cword[j], runtime->cword[i]);
                if ( r > 0) {
...
BOOL mrdc_icli_exec_cword_runtime_get(
    IN  icli_session_handle_t   *handle,
...
)
{
    ...
    b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_CWORD, runtime );
    if ( b ) {
        mrdc_icli_exec_cword_runtime_sort( runtime );
    }
    return b;
}
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_exec_cword_match_cnt(
    IN  icli_session_handle_t   *handle,
    IN  const char              *word,
    IN  icli_runtime_cb_t       *runtime_cb,
    OUT icli_runtime_t          *runtime,
...
)
{
    ...
    b = mrdc_icli_exec_cword_runtime_get(handle, runtime_cb, runtime);
    if ( b == FALSE ) {
        return -1;
    }
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        r = mrdc_icli_str_sub(word, runtime->cword[i], 0, NULL);
        switch ( r ) {
        case 1:
            ++sub_cnt;
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_exec_priv_parsing(
    IN  icli_session_handle_t   *handle,
...
extern "C"
#endif
i32 mrdc_icli_exec_para_cnt_get(
    void
)
{
    return g_para_cnt;
...
char *mrdc_icli_exec_para_word_get(
    IN  icli_parameter_t    *p,
    IN  char                *s
)
{
    ...
    case ICLI_VARIABLE_MAC_MCAST:
        (void)mrdc_icli_mac_to_str(p->value.u.u_mac_addr.addr, c);
        break;
    ...
    case ICLI_VARIABLE_IPV4_ABC:
        (void)mrdc_icli_ipv4_to_str(p->value.u.u_ipv4_addr, c);
        break;

    /* icli_ipv4_subnet_t */
    case ICLI_VARIABLE_IPV4_SUBNET:
        (void)mrdc_icli_ipv4_to_str(p->value.u.u_ipv4_subnet.ip, c);
        c = c + mrdc_icli_str_len(c);
        (void)icli_sprintf(c, "/");
        ++c;
        (void)mrdc_icli_ipv4_to_str(p->value.u.u_ipv4_subnet.netmask, c);
        break;
    ...
    case ICLI_VARIABLE_IPV6_LINKLOCAL:
        (void)mrdc_icli_ipv6_to_str(p->value.u.u_ipv6_addr, c);
        break;

    /* mrdc_ipv6_network_t */
    case ICLI_VARIABLE_IPV6_SUBNET:
        (void)mrdc_icli_ipv6_to_str(p->value.u.u_ipv6_subnet.address, c);
        c = c + mrdc_icli_str_len(c);
        (void)icli_sprintf(c, "/");
        ++c;
    ...
    case ICLI_VARIABLE_RANGE_URL_FILE:
        (void)mrdc_icli_str_cpy(c, p->value.u.u_word);
        break;
    ...
    case ICLI_VARIABLE_PORT_TYPE_ID:
        (void)mrdc_icli_switch_port_range_to_str(&(p->value.u.u_port_id), c);
        break;

    /* icli_stack_port_range_t */
    case ICLI_VARIABLE_PORT_TYPE_LIST:
        (void)mrdc_icli_stack_port_range_to_str(&(p->value.u.u_port_list), c);
        break;
    ...
    case ICLI_VARIABLE_SWITCH_LIST:
    case ICLI_VARIABLE_RANGE_UINT_LIST:
        (void)mrdc_icli_unsigned_range_to_str(&(p->value.u.u_vlan_list), c);
        break;

    /* icli_signed_range_t */
    case ICLI_VARIABLE_RANGE_INT_LIST:
        (void)mrdc_icli_signed_range_to_str(&(p->value.u.u_range_int_list), c);
        break;

    /* icli_range_t */
    case ICLI_VARIABLE_RANGE_LIST:
        (void)mrdc_icli_range_to_str(&(p->value.u.u_range_list), c);
        break;

    /* icli_oui_t */
    case ICLI_VARIABLE_OUI:
        (void)mrdc_icli_oui_to_str(&(p->value.u.u_oui), c);
        break;

    /* icli_clock_id_t */
    case ICLI_VARIABLE_CLOCK_ID:
        (void)mrdc_icli_clock_id_to_str(&(p->value.u.u_clock_id), c);
        break;

    /* icli_vcap_vr_t */
    case ICLI_VARIABLE_VCAP_VR:
        (void)mrdc_icli_vcap_vr_to_str(&(p->value.u.u_vcap_vr), c);
        break;

    /* icli_hexval_t */
    case ICLI_VARIABLE_HEXVAL:
    case ICLI_VARIABLE_RANGE_HEXVAL:
        (void)mrdc_icli_hexval_to_str(&(p->value.u.u_hexval), c);
        break;

    /* mrdc_ip_t */
    case ICLI_VARIABLE_AREA_ID:
        (void)mrdc_icli_ipv4_to_str(p->value.u.u_area_id, c);
        break;
...
char *mrdc_icli_exec_full_cmd_str_get(
    IN  icli_session_handle_t   *handle
)
{
    icli_parameter_t    *p;
    ...
    */
    ...
    for (p = handle->runtime_data.cmd_var; p; ___NEXT(p)) {
        if (mrdc_icli_exec_para_word_get(p, c)) {
...
void mrdc_icli_exec_full_cmd_str_get_end(
    IN  icli_session_handle_t   *handle
)
{
    if (handle) {
...
u32 mrdc_icli_exec_highest_para_priv_get(
    IN  icli_parameter_t    *p
)
{
    u32                     cmd_priv;
...
u32 mrdc_icli_exec_highest_cmd_priv_get(
    IN  icli_session_handle_t   *handle
)
{
    icli_parameter_t        *p;
    ...
    return mrdc_icli_exec_highest_para_priv_get(p);
}
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_exec_visible_chk(
    IN  icli_session_handle_t   *handle,
    IN  icli_parameter_t        *para
)
{
