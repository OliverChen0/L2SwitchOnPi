        ...
        case ICLI_WORD_TYPE_OPTIONAL_BEGIN:
            ...
            for (;;) {
                ...
                *(cmd - 1) = ICLI_EOS;
                ...
                i = mrdc_icli_word_count(sub_cmd);
            ...
            if ( symbol == ICLI_MANDATORY_END ) {
                if ( ICLI_IS_RANDOM_MUST(cmd - 1) ) {
                    if ( mrdc_icli_parsing_random_head(&g_cmd_node[current_begin_id], &g_cmd_node[current_end_id], &must_level) ) {
                        /* get must number */
                        must_number = *(cmd + 1) - '0';
...
)
{
    ...
    */
    ...
    // word
    if ( mrdc_icli_str_cmp(n1->word, n2->word) != 0 ) {
        return FALSE;
    }
    // optional_begin
                ...
                for ( t = tree; t; ___SIBLING(t) ) {
                    if ( _node_is_same(n, t) ) {
                        ...
                        T_E("Duplicate word \"%s\". "
                            ...
                            n->word,
                            mrdc_icli_register_cmd_str_get(t->node_property.cmd_property->cmd_id),
            ...
            if ( b_loop ) {
                if ( loop_overlap_cnt ) {
                    T_E("Word \"%s\" in loop not overlapped, but it should.\n"
                        ...
                        "*** Please refer to AN1047, Appendix B.1\n",
                        w,
                        mrdc_icli_register_cmd_str_get(pre_tree->node_property.cmd_property->cmd_id),
                        cmd_register->original_cmd);
                    return ICLI_RC_ERROR;
                } else {
                    // restore loop syntax
...
)
{
    if ( mode >= ICLI_CMD_MODE_MAX ) {
        T_E("mrdc_icli_parsing_tree_get : invalid mode = %d\n", mode);
        return NULL;
    }
