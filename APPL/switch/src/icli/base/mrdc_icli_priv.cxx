...
)
{
    ...
    (void)mrdc_icli_str_cpy( cmd, fcmd );
    icli_free( fcmd );
}
...
)
{
    ...
    p = g_cmd_list;
    do {
        if ( p->conf.mode == conf->mode && mrdc_icli_str_cmp(p->conf.cmd, conf->cmd) == 0 ) {
            return p;
        }
        ___NEXT( p );
    } while ( p != g_cmd_list );
...
)
{
    ...
    */
    ...
    rc = mrdc_icli_exec_priv_parsing( handle, conf, match_node );
    if ( rc != ICLI_RC_OK ) {
        return rc;
    }
