...
#ifdef MRDC_SW_OPTION_SYSLOG
#include "syslog_history.h"
#endif
...
MRDC_ENUM_INC(icli_port_type_t);
#endif
...
)
{
    ...
    while ( rc ) {
        rc = mrdc_icli_session_mode_exit( handle->session_id );
        if ( rc < 0 ) {
            break;
        }
    }
...
)
{
    ...
    if ( handle->runtime_data.match_para ) {
        mrdc_icli_exec_para_list_free( &(handle->runtime_data.match_para) );
    }
...
static void _history_cmd_format(IN icli_session_handle_t *handle, OUT char *buf, IN int length)
{
    static int header_no = g_ramdon % 1000000;
    char user_info[128];
    if (handle->session_id > 0) {
        char ip[64];
        if (handle->open_data.client_ip.type == MRDC_IP_TYPE_IPV4) {
            (void)mrdc_icli_ipv4_to_str(handle->open_data.client_ip.addr.ipv4, ip);
        } else {
            (void)mrdc_icli_ipv6_to_str(handle->open_data.client_ip.addr.ipv6, ip);
        }
        snprintf(user_info, sizeof(user_info), "%s, vty%d, %s", handle->runtime_data.user_name, handle->session_id, ip);
    } else {
        snprintf(user_info, sizeof(user_info), "%s, console", handle->runtime_data.user_name);
    }

    const char *timer = misc_time2str(time(NULL));

    snprintf(buf, length, "[Index] : %d User info : %s Time : %s Command : %s\n",
             header_no++, user_info, timer, handle->runtime_data.cmd);
}
...
)
{
    ...
    /* empty command */
    if ( mrdc_icli_str_len(handle->runtime_data.cmd) == 0 ) {
        return;
    }

    /* command is the same with the latest history */
    if ( ! mrdc_icli_sutil_history_cmd_empty(handle) ) {
        if ( mrdc_icli_str_cmp(handle->runtime_data.cmd, handle->runtime_data.history_cmd_head->cmd) == 0 ) {
            return;
        }
    }

#ifdef MRDC_SW_OPTION_SYSLOG
    char history_format[ICLI_STR_MAX_LEN + 256];
    _history_cmd_format(handle, history_format, sizeof(history_format));
    T_N("%s", history_format);
    add_history_log(history_format);
#endif

    if ( mrdc_icli_sutil_history_cmd_cnt(handle) >= ICLI_HISTORY_MAX_CNT ) {
        ___NEXT( handle->runtime_data.history_cmd_head );
    } else {
        hcmd = mrdc_icli_sutil_history_cmd_alloc( handle );
        if ( hcmd ) {
    ...
    (void)mrdc_icli_str_cpy(handle->runtime_data.history_cmd_head->cmd, handle->runtime_data.cmd);
}
...
)
{
    ...
    case ICLI_INPUT_STYLE_SINGLE_LINE :
        mrdc_icli_c_cmd_display( handle );
        break;

    case ICLI_INPUT_STYLE_MULTIPLE_LINE :
        mrdc_icli_a_cmd_display( handle );
        break;

    case ICLI_INPUT_STYLE_SIMPLE :
        mrdc_icli_z_cmd_display( handle );
        break;
...
)
{
    ...
    switch ( _SESSON_INPUT_STYLE ) {
    case ICLI_INPUT_STYLE_SINGLE_LINE :
        mrdc_icli_c_cmd_redisplay( handle );
        break;

    case ICLI_INPUT_STYLE_MULTIPLE_LINE :
        mrdc_icli_a_cmd_redisplay( handle );
        break;
...
)
{
    ...
    switch ( _SESSON_INPUT_STYLE ) {
    case ICLI_INPUT_STYLE_SINGLE_LINE :
        rc = mrdc_icli_c_usr_cmd_get( handle );
        break;

    case ICLI_INPUT_STYLE_MULTIPLE_LINE :
        rc = mrdc_icli_a_usr_cmd_get( handle );
        break;

    case ICLI_INPUT_STYLE_SIMPLE :
        rc = mrdc_icli_z_usr_cmd_get( handle );
        //line count
...
)
{
    ...
    mode_prompt = mrdc_icli_mode_prompt_get( mode );
    ...
    if (switch_info.b_primary) {
        ...
        if (mrdc_icli_str_len(mode_prompt)) {
    ...
    mrdc_icli_session_str_put(handle, full_prompt);
    handle->runtime_data.prompt_len = mrdc_icli_str_len(full_prompt);
}
...
)
{
    ...
    if ( node->type != ICLI_VARIABLE_KEYWORD ) {
        ...
        if ( runtime_cb ) {
            b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_BYWORD, &(handle->runtime_data.runtime) );
            if ( b ) {
    ...
    case ICLI_VARIABLE_LOOPBACK_ID:
    ...
    case ICLI_VARIABLE_RANGE_UINT_LIST:
        ...
        runtime_cb = node->node_property.cmd_property->runtime_cb[node->word_id];
        if ( runtime_cb ) {
            b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_RANGE, &(handle->runtime_data.runtime) );
            if ( b ) {
                u32                     i;
                icli_range_t            range;
        ...
        bw = &(node->node_property.cmd_property->byword[node->word_id]);
        byword = bw->word;
        if ( mrdc_icli_str_len(byword) && bw->para_cnt ) {
            if ( bw->para_cnt != -1 ) {
                /*
                    this byword does not need to be freed because it is linked
                    into node_property of the parsing tree and it can be used
...
)
{
    ...
    b = mrdc_icli_exec_cword_runtime_get(handle, runtime_cb, &(handle->runtime_data.runtime));
    if ( b == FALSE ) {
        return -1;
    }
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        if ( *cword ) {
            if ( mrdc_icli_str_sub(cword, handle->runtime_data.runtime.cword[i], 0, NULL) == -1 ) {
                continue;
            }
        }
        len = mrdc_icli_str_len( handle->runtime_data.runtime.cword[i] );
        if ( len > max_len ) {
            max_len = len;
        }
    }
...
)
{
    ...
    for ( p = handle->runtime_data.match_sort_list; p != NULL; p = p->next ) {
        ...
        case ICLI_VARIABLE_PORT_TYPE:
            for ( port_type = (icli_port_type_t)1; port_type < ICLI_PORT_TYPE_MAX; ++port_type ) {
                if ( b_use_exec_present ) {
                    if ( mrdc_icli_exec_port_type_present(handle, p->match_node, port_type) ) {
                        l = mrdc_icli_str_len( mrdc_icli_variable_port_type_get_name(port_type) );
                        if ( l > len ) {
                            len = l;
                        }
                    }
                } else {
                    if ( mrdc_icli_port_type_present(port_type) ) {
                        l = mrdc_icli_str_len( mrdc_icli_variable_port_type_get_name(port_type) );
                        if ( l > len ) {
                            len = l;
                        }
        ...
        case ICLI_VARIABLE_CWORD:
            if ( b_use_exec_present ) {
                ...
                if ( len == -1 ) {
                    len = mrdc_icli_str_len( _cmd_word_get(handle, p->match_node) );
                }
            } else {
                len = mrdc_icli_str_len("<cword>");
            }
            break;
    ...
    if ( handle->runtime_data.b_cr ) {
        if ( (i32)mrdc_icli_str_len(__ICLI_CR_STR) > max_word_len ) {
            max_word_len = mrdc_icli_str_len( __ICLI_CR_STR );
        }
    }
    return max_word_len;
}
...
)
{
    ...
    switch ( _SESSON_INPUT_STYLE ) {
    case ICLI_INPUT_STYLE_SINGLE_LINE :
        mrdc_icli_c_line_clear( handle );
        break;

    case ICLI_INPUT_STYLE_MULTIPLE_LINE :
        mrdc_icli_a_line_clear( handle );
        break;

    case ICLI_INPUT_STYLE_SIMPLE :
        mrdc_icli_z_line_clear( handle );
        break;
...
#define __TAB_WORD_DISPLAY(word) \
    ...
    w = word; \
    mrdc_icli_session_str_put(handle, w); \
    j = max_word_len - mrdc_icli_str_len(w); \
    for ( ; j > 0; --j ) { \
        ICLI_PUT_SPACE; \
    } \
    mrdc_icli_session_str_put(handle, "  "); \
    ++i;
...
)
{
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        r = mrdc_icli_str_sub(cword, handle->runtime_data.runtime.cword[i], 0, NULL);
        switch ( r ) {
        case 1:
            ++sub_cnt;
            break;
...
)
{
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        case 0:
            (void)mrdc_icli_str_cpy(w, handle->runtime_data.runtime.cword[i]);
            return;
    ...
    if ( k >= 0 ) {
        (void)mrdc_icli_str_cpy(w, handle->runtime_data.runtime.cword[k]);
    }
}
...
)
{
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        if ( c ) {
            len = mrdc_icli_str_prefix( handle->runtime_data.runtime.cword[i], c, len, 0);
        } else {
...
)
{
    ...
    runtime_cb = p->match_node->node_property.cmd_property->runtime_cb[p->word_id];
    return mrdc_icli_exec_cword_match_cnt( handle, p->value.u.u_cword, runtime_cb, runtime, NULL, NULL );
}
...
)
{
    ...
    if (b_space && handle->runtime_data.b_cr) {
        if (icli_platform_cmd_authorize(handle,
                                        mrdc_icli_exec_full_cmd_str_get(handle),
                                        mrdc_icli_exec_highest_cmd_priv_get(handle),
...
_aaa_match_list_restart:
    for (prev_p = NULL, p = handle->runtime_data.match_sort_list; p; prev_p = p, ___NEXT(p)) {
        ...
        command = mrdc_icli_exec_full_cmd_str_get(handle);

        // append space
        c = command;
        if (mrdc_icli_str_len(c)) {
        ...
        // AAA
        switch (icli_platform_cmd_authorize(handle, command, mrdc_icli_exec_highest_para_priv_get(p), FALSE)) {
        case ICLI_RC_ERR_AAA_REJECT:
            ...
            if (prev_p) {
                prev_p->next = p->next;
                mrdc_icli_exec_parameter_free(p);
                p = prev_p;
            } else {
    ...
    if (handle->runtime_data.b_cr == FALSE && b_match_list && handle->runtime_data.match_sort_list == NULL) {
        ...
        mrdc_icli_session_str_put(handle, "% Command authorization failed.\n");
        return FALSE;
    }
...
)
{
    ...
    while ( p ) {
        if ( mrdc_icli_exec_visible_chk(handle, p) ) {
            prev = p;
            p = p->next;
        } else {
            q = p;
            ...
            mrdc_icli_exec_parameter_free( q );
        }
    }
}
#endif
...
)
{
    ...
    if ( keyword ) {
        ...
        if ( (p->match_node->type == ICLI_VARIABLE_PORT_TYPE) &&
            ...
            handle->runtime_data.tab_port_type = mrdc_icli_port_next_present_type( ICLI_PORT_TYPE_NONE );
        }
        ...
        switch (p->match_node->type) {
        case ICLI_VARIABLE_KEYWORD:
            (void)mrdc_icli_str_cpy(w, p->match_node->word);
            break;

        case ICLI_VARIABLE_PORT_TYPE:
            if ( p->value.type == ICLI_VARIABLE_PORT_TYPE ) {
                (void)mrdc_icli_str_cpy(w, mrdc_icli_variable_port_type_get_name(p->value.u.u_port_type));
            } else {
                (void)mrdc_icli_str_cpy(w, mrdc_icli_variable_port_type_get_name((icli_port_type_t)(handle->runtime_data.tab_port_type)));
            }
            break;

        case ICLI_VARIABLE_GREP:
            (void)mrdc_icli_str_cpy(w, ICLI_GREP_KEYWORD);
            break;

        case ICLI_VARIABLE_GREP_BEGIN:
            (void)mrdc_icli_str_cpy(w, ICLI_GREP_BEGIN_KEYWORD);
            break;

        case ICLI_VARIABLE_GREP_INCLUDE:
            (void)mrdc_icli_str_cpy(w, ICLI_GREP_INCLUDE_KEYWORD);
            break;

        case ICLI_VARIABLE_GREP_EXCLUDE:
            (void)mrdc_icli_str_cpy(w, ICLI_GREP_EXCLUDE_KEYWORD);
            break;
        ...
        *w   = ICLI_EOS;
        ...
        cmd_len = mrdc_icli_str_len( handle->runtime_data.cmd );
        ...
        if ( handle->runtime_data.match_sort_list ) {
                    ...
                    for ( port_type = start_type; port_type < ICLI_PORT_TYPE_MAX; ++port_type ) {
                        if ( mrdc_icli_exec_port_type_present(handle, p->match_node, port_type) ) {
                            if ( (p->value.type == ICLI_VARIABLE_MAX) ||
                                ...
                                __TAB_WORD_SET( mrdc_icli_variable_port_type_get_name(port_type) );
                            }
                        }
                    }
#else
            ...
            mrdc_icli_exec_cword_runtime_sort( &irt );
            for ( k = 0; k < ICLI_CWORD_MAX_CNT; ++k ) {
                if ( irt.cword[k] == NULL ) {
                    break;
                }
        ...
        if ( handle->runtime_data.b_cr ) {
            ...
            mrdc_icli_session_str_put(handle, __ICLI_CR_STR);
#if 1 /* CP, 2014-10-28 14:12, Bugzilla#16112 */
            b_print = TRUE;
#endif
        }
        ...
        */
        ...
        if ( handle->runtime_data.match_sort_list ) {
            ...
            if ( p->match_node->type == ICLI_VARIABLE_KEYWORD ) {
                c = p->match_node->word;
                prefix_num = mrdc_icli_str_len( c );
            } else {
                prefix_num = 0;
            ...
            for ( ___NEXT(p); p != NULL; ___NEXT(p) ) {
                if ( p->match_node->type == ICLI_VARIABLE_KEYWORD ) {
                    prefix_num = mrdc_icli_str_prefix( p->match_node->word, c, prefix_num, 0);
                } else {
                    ...
                    prefix_num = mrdc_icli_str_prefix( s, c, prefix_num, 0);
                }
            }

            j = mrdc_icli_str_len(w);
            i = prefix_num - j;
            c = c + j;
...
)
{
    ...
    runtime_cb = node->node_property.cmd_property->runtime_cb[node->word_id];
    if ( runtime_cb ) {
        b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_HELP, &(handle->runtime_data.runtime) );
        if ( b ) {
...
)
{
    ...
    (void)mrdc_icli_session_va_str_put(handle->session_id, format, arglist);
    va_end( arglist );
}
...
)
{
    ...
    for ( i = mrdc_icli_str_len(word); i < max_word_len; ++i ) {
        _session_printf(handle, " ");
    }

    //help
    help_len = mrdc_icli_str_len( help );
...
)
{
    ...
    /* get port range */
    if ( mrdc_icli_port_range_get(&range) == FALSE ) {
        return NULL;
    }
...
)
{
    ...
    s = cmd_help;
    if ( b_port_list ) {
        (void)mrdc_icli_str_cpy(s, "Port list in ");
    } else {
        (void)mrdc_icli_str_cpy(s, "Port ID in ");
    }
    s = cmd_help + mrdc_icli_str_len( cmd_help );

    /* get port list string */
    if ( handle->runtime_data.last_port_type == ICLI_PORT_TYPE_ALL ) {
        (void)mrdc_icli_str_cpy( cmd_help, "Port list for all port types" );
    } else {
...
)
{
    ...
    for ( i = 0; i < ICLI_CWORD_MAX_CNT; ++i ) {
        ...
        if ( para->value.u.u_cword[0] ) {
            if ( mrdc_icli_str_sub(para->value.u.u_cword, runtime->cword[i], 0, NULL) < 0 ) {
                continue;
            }
        }
...
)
{
    ...
    s = cmd_help;
    if ( b_list ) {
        (void)mrdc_icli_str_cpy(s, "Switch ID list in ");
    } else {
        (void)mrdc_icli_str_cpy(s, "Switch ID in ");
    }
    s = cmd_help + mrdc_icli_str_len( cmd_help );
    ...
    begin_switch_id = 0;
    end_switch_id   = 0;
    while ( mrdc_icli_switch_get_next(&spr) ) {
        if ( end_switch_id ) {
...
#define __PORT_TYPE_HELP(t) \
    if ( mrdc_icli_port_type_present(t) ) { \
        if ( (p->value.type == ICLI_VARIABLE_MAX) || \
             (p->value.type == ICLI_VARIABLE_PORT_TYPE && p->value.u.u_port_type == t)) { \
            _help_display(handle, mrdc_icli_variable_port_type_get_name(t), mrdc_icli_variable_port_type_get_help(t));\
        } \
    }
...
)
{
    ...
    for ( p = handle->runtime_data.match_sort_list; p != NULL; p = p->next) {
        ...
        (void)mrdc_icli_str_ncpy(cmd_word, _cmd_word_get(handle, p->match_node), ICLI_BYWORD_MAX_LEN);
        (void)mrdc_icli_str_ncpy(cmd_help, _help_get(handle, p->match_node, &b_runtime), ICLI_HELP_MAX_LEN);
        ...
        runtime_cb = p->match_node->node_property.cmd_property->runtime_cb[p->word_id];
        if ( runtime_cb ) {
            b = mrdc_icli_exec_runtime( handle, runtime_cb, ICLI_ASK_PORT_RANGE, &(handle->runtime_data.runtime) );
            if ( b ) {
                /* backup system port range */
                mrdc_icli_port_range_backup();

                /* set system port range for temp use */
                if ( mrdc_icli_port_range_set( &(handle->runtime_data.runtime.port_range) ) == FALSE ) {
        ...
        case ICLI_VARIABLE_CWORD:
            ...
            if ( mrdc_icli_exec_cword_runtime_get(handle, runtime_cb, &(handle->runtime_data.runtime)) ) {
...
)
{
    ...
    switch ( _SESSON_INPUT_STYLE ) {
    case ICLI_INPUT_STYLE_SINGLE_LINE :
        rc = mrdc_icli_c_line_mode( handle );
        break;

    case ICLI_INPUT_STYLE_MULTIPLE_LINE :
        rc = mrdc_icli_a_line_mode( handle );
        break;

    case ICLI_INPUT_STYLE_SIMPLE :
        rc = mrdc_icli_z_line_mode( handle );
        break;
...
)
{
    ...
    for ( c = l; ICLI_NOT_(EOS, *c); ++c ) {
        ...
        if ( p ) {
            ...
            case ICLI_VARIABLE_GREP_BEGIN:
                if ( handle->runtime_data.grep_begin == 0 ) {
                    if ( mrdc_icli_str_str(p->next->next->value.u.u_line, l) ) {
            ...
            case ICLI_VARIABLE_GREP_INCLUDE:
                if ( mrdc_icli_str_str(p->next->next->value.u.u_line, l) == NULL ) {
                    b_print = FALSE;
                }
                break;
        ...
        /* if print this line */
        if ( b_print == FALSE ) {
            (void)mrdc_icli_str_cpy(handle->runtime_data.str_buf, c + 1);
            /* -1 because c++ in for() */
            c = l;
            --c;
            continue;
        ...
        *c = ICLI_EOS;
        ...
        if ( handle->runtime_data.line_mode == ICLI_LINE_MODE_PAGE && handle->runtime_data.lines ) {
            ...
            line_cnt = ( mrdc_icli_str_len(l) - 1 ) / handle->runtime_data.width + 1;
    ...
    *c = x;
    if ( x ) {
        (void)mrdc_icli_str_cpy(handle->runtime_data.str_buf, c);
    } else {
        memset(handle->runtime_data.str_buf, 0, sizeof(handle->runtime_data.str_buf));
    }
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_init(void)
{
    u32                 i;

#if 1 /* CP, 2012/09/14 10:38, conf */
    icli_conf_data_t    *conf = mrdc_icli_conf_get();
#endif
    ...
    for ( i = 0; i < ICLI_SESSION_CNT; ++i ) {
        ...
        (void)mrdc_icli_session_config_data_default( g_session_handle[i].config_data );
        _runtime_data_default( &(g_session_handle[i]) );
        g_session_handle[i].session_id = i;
    }
    g_max_sessions = ICLI_SESSION_DEFAULT_CNT;
    mrdc_random(&g_ramdon);
    return ICLI_RC_OK;
}
...
#define __AUTH_INPUT_AND_CHECK(s, t)\
    size = sizeof(s);\
    memset(s, 0, size);\
    rc = mrdc_icli_session_usr_str_get(session_id, t, s, &size, NULL);\
    /* timeout */\
    if ( rc != ICLI_RC_OK ) {\
...
static mrdc_rc __cli_io_getch(cli_iolayer_t *pIO, int secs, char *ch)
{
    fd_set fds;
    struct timeval tv;
    ...
    while ( handle->runtime_data.alive ) {
        ...
        if ( do_auth ) {
            ...
            if ( handle->config_data->b_motd_banner ) {
                banner = mrdc_icli_banner_motd_get();
                if ( mrdc_icli_str_len(banner) ) {
                    ICLI_PUT_NEWLINE;
                    mrdc_icli_session_str_put(handle, banner);
                    ICLI_PUT_NEWLINE;
                }
            }
            ...
            case ICLI_SESSION_WAY_CONSOLE:
                if ( auth_cnt == 0 ) {
                    ICLI_PUT_NEWLINE;
                    mrdc_icli_session_str_put(handle, "press ENTER to get started");
                    for ( b = TRUE, c = 0;
                          b && ICLI_NOT_(KEY_ENTER, c) && ICLI_NOT_(NEWLINE, c);
                          b = mrdc_icli_sutil_usr_char_get(handle, ICLI_TIMEOUT_FOREVER, &c) ) {
                        ;
                    }
                    if ( b == FALSE ) {
                        icli_os_sleep( 1000 );
            ...
            case ICLI_SESSION_WAY_TELNET:
                ...
                banner = mrdc_icli_banner_login_get();
                if ( mrdc_icli_str_len(banner) ) {
                    ICLI_PUT_NEWLINE;
                    mrdc_icli_session_str_put(handle, banner);
                    ICLI_PUT_NEWLINE;
                ...
                mrdc_icli_session_str_put(handle, "Username: ");
                __AUTH_INPUT_AND_CHECK(username, ICLI_USR_INPUT_TYPE_NORMAL);

                mrdc_icli_session_str_put(handle, "Password: ");
                ...
                if ( rc != ICLI_RC_OK ) {
                    mrdc_icli_session_str_put(handle, "Wrong username or password!\n\n");
                ...
                (void)mrdc_icli_str_ncpy( handle->runtime_data.user_name, username, ICLI_USERNAME_MAX_LEN );
                break;
            ...
            if ( handle->config_data->b_exec_banner ) {
                banner = mrdc_icli_banner_exec_get();
                if ( mrdc_icli_str_len(banner) ) {
                    ICLI_PUT_NEWLINE;
                    mrdc_icli_session_str_put(handle, banner);
                    ICLI_PUT_NEWLINE;
        ...
        (void) mrdc_icli_session_mode_get(session_id, &mode);
        ...
        case ICLI_RC_ERR_EMPTY:
            ...
            case ICLI_AFTER_CMD_ACT_GOTO_PREVIOUS_MODE:
                (void)mrdc_icli_session_mode_exit( handle->session_id );
                break;
        ...
        case ICLI_SESSION_WAY_CONSOLE:
            if ( handle->runtime_data.alive == FALSE ) {
                if ( mrdc_icli_console_alive_get() ) {
                    /* redo auth */
                    do_auth  = TRUE;
                    auth_cnt = 0;
        ...
        case ICLI_SESSION_WAY_APP_EXEC:
            T_I("invalid type %d of session ID = %d\n", handle->open_data.way, session_id);
            return ICLI_RC_ERROR;
        default:
            T_E("invalid type %d of session ID = %d\n", handle->open_data.way, session_id);
            return ICLI_RC_ERROR;
...
}// mrdc_icli_session_open

/*
    close a ICLI session

    INPUT
        session_id : session ID from mrdc_icli_session_open()
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_session_close_by_username(
    IN char     *username
)
{
    u32     i;

    if ( username == NULL || mrdc_icli_str_len(username) == 0 ) {
        return;
    }

    for ( i = 0; i < g_max_sessions; ++i ) {
        if ( mrdc_icli_str_cmp(username, g_session_handle[i].runtime_data.user_name) == 0 ) {
            (void)mrdc_icli_session_close( i );
        }
    }
}
...
/*
    ...
    COMMENT
        n/a
*/
icli_session_handle_t *mrdc_icli_session_handle_get(
    IN u32  session_id
)
{
    if ( session_id >= ICLI_SESSION_CNT ) {
...
/*
    ...
    COMMENT
        n/a
*/
u32 mrdc_icli_session_max_cnt_get(void)
{
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_max_cnt_set(
    IN u32 max_sessions
)
{
    u32     i;
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_mode_get(
    IN  u32                 session_id,
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_mode_enter(
    IN  u32                 session_id,
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_mode_exit(
    IN  u32     session_id
)
{
    icli_session_handle_t   *handle;
    ...
    /* pack mode */
    mode_para = &( handle->runtime_data.mode_para[handle->runtime_data.mode_level] );
    mrdc_icli_exec_para_list_free( &(mode_para->cmd_var) );
...
/*
    ...
    COMMENT
        none
*/
i32 mrdc_icli_session_va_str_put(
    IN u32          session_id,
    IN const char   *format,
    IN va_list      arglist
)
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_usr_str_get(
    IN      u32                     session_id,
...
)
{
    ...
    while ( loop == 1 ) {
        ...
        case ICLI_USR_INPUT_TYPE_PASSWORD:
            ...
            if ( mrdc_icli_sutil_usr_char_get(handle, ICLI_ENABLE_PASSWORD_WAIT, &c) == FALSE ) {
                return ICLI_RC_ERR_EXPIRED;
            }
            break;
        }
        ...
        */
        ...
        /*
            ...
            Bug: if copy-paste, then the first char will always be consumed.
                 so, this is not good.
        */
        //mrdc_icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c);
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_ctrl_c_get(
    IN  u32     session_id,
    IN  u32     wait_time
)
{
    ...
    while (wait_time > 0) {
        ...
        /* wait Ctrl-C */
        if ( mrdc_icli_sutil_usr_char_get(handle, (i32)wait_time, &c) == FALSE ) {
            return ICLI_RC_ERR_EXPIRED;
        }
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_data_get(
...
)
{
    ...
    (void)mrdc_icli_str_ncpy(data->location, handle->config_data->location, ICLI_LOCATION_MAX_LEN);
    data->privileged_level = (icli_privilege_t)(handle->config_data->privileged_level);
#endif

#if 1 /* CP, 2012/09/04 16:46, session user name */
    (void)mrdc_icli_str_ncpy(data->user_name, handle->runtime_data.user_name, ICLI_USERNAME_MAX_LEN);
...
/*
    ...
    COMMENT
        n/a
*/
i32 mrdc_icli_session_data_get_next(
    INOUT icli_session_data_t   *data
)
{
    if ( data == NULL ) {
    ...
    /* get session data */
    return mrdc_icli_session_data_get( data );
}
...
/*
    ...
    RETURN
        n/a
*/
void mrdc_icli_session_char_put(
    IN  icli_session_handle_t   *handle,
    IN  char                    c
)
{
    ...
    case ICLI_ERR_DISPLAY_MODE_PRINT :
        (void)mrdc_icli_sutil_usr_char_put(handle, c);
        break;

    case ICLI_ERR_DISPLAY_MODE_ERR_BUFFER :
        len = mrdc_icli_str_len( handle->runtime_data.err_buf );
...
/*
    ...
    COMMENT
        none
*/
void mrdc_icli_session_str_put(
    IN  icli_session_handle_t   *handle,
    IN  const char              *str
)
{
    ...
    case ICLI_ERR_DISPLAY_MODE_ERR_BUFFER :
        ...
        if ( mrdc_icli_str_concat(handle->runtime_data.err_buf, str) == NULL ) {
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_session_str_printf(
    IN  icli_session_handle_t   *handle,
    IN  const char              *format,
    IN  ...
)
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_session_error_printf(
    IN  icli_session_handle_t   *handle,
    IN  const char              *format,
    IN  ...
)
{
    ...
    if ( handle->runtime_data.b_cmd_copy ) {
        ...
        len = mrdc_icli_str_len( handle->runtime_data.cmd_copy );
    }
...
mrdc_rc mrdc_icli_session_multiline_data_get(u32 session_id, icli_session_multiline_data_t *const data)
{
    icli_session_handle_t *handle = mrdc_icli_session_handle_get(session_id);
...
mrdc_rc mrdc_icli_session_multiline_data_set(u32 session_id, const icli_session_multiline_data_t *data)
{
    icli_session_handle_t *handle = mrdc_icli_session_handle_get(session_id);
...
void mrdc_icli_session_multiline_data_clear(u32 session_id)
{
    icli_session_handle_t *handle = mrdc_icli_session_handle_get(session_id);
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_session_config_data_default(
...
)
{
    ...
    /* set to default */
    config_data->input_style      = mrdc_icli_input_style_get();
...
/*
    ...
    COMMENT
        n/a
*/
void mrdc_icli_session_debug_cmd_allow_set(
    IN  icli_session_handle_t   *handle,
    IN  BOOL                    b_allow
)
{
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_session_debug_cmd_allow_get(
    IN  icli_session_handle_t   *handle
)
{
    if ( handle == FALSE ) {
...
/*
    ...
    COMMENT
        n/a
*/
BOOL mrdc_icli_session_debug_cmd_allow_first_time_get(
    IN  icli_session_handle_t   *handle
)
{
    if ( handle == FALSE ) {
...
*/
i32 mrdc_icli_session_mode_para_get(
    IN  u32                     session_id,
    OUT icli_cmd_mode_t         *mode,
