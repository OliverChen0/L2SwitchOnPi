...
)
{
    ...
    // get end x,y
    mrdc_icli_sutil_end_xy_get(handle, &ex, &ey);
...
)
{
    ...
    if ( _CURSOR_AT_CMD_END ) {
        ...
        // get current x,y
        mrdc_icli_sutil_current_xy_get(handle, &nx, &ny);
...
)
{
    ...
    // print rest string
    mrdc_icli_session_str_put(handle, handle->runtime_data.cmd + cmd_pos);
    ...
    // update command
    (void)mrdc_icli_str_cpy(handle->runtime_data.cmd, handle->runtime_data.cmd + cmd_pos);
...
)
{
    ...
    /* add char */
    if ( mrdc_icli_sutil_cmd_char_add(handle, c) == FALSE ) {
        return;
    }

    /* check if any char already in buffer */
    if ( mrdc_icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c) ) {
        b_buf = TRUE;
        while ( ICLI_NOT_(KEY_ENTER, c) && ICLI_NOT_(NEWLINE, c)) {
            (void)mrdc_icli_sutil_cmd_char_add(handle, c);
            if ( mrdc_icli_sutil_usr_char_get(handle, _BUF_WAIT_TIME, &c) == FALSE ) {
                /* no char in buffer */
                b_buf = FALSE;
                break;
            }
        ...
        mrdc_icli_session_str_put(handle, handle->runtime_data.cmd + handle->runtime_data.cursor_pos);
        // update cursor_pos
        handle->runtime_data.cursor_pos = handle->runtime_data.cmd_pos;
        // update cursor
        if ( ! _CURSOR_AT_CMD_END ) {
            ...
            mrdc_icli_sutil_current_xy_get(handle, &x, &y);
            // get end x,y
            mrdc_icli_sutil_end_xy_get(handle, &nx, &ny);
            // go back to the original x,y
            icli_platform_cursor_offset(handle, x - nx, y - ny);
        }
    } else {
    ...
    while ( loop ) {
        ...
        */
        ...
        case ICLI_HYPER_TERMINAL:
            ...
            case ICLI_VKEY_HOME:
                //get next c
                if ( mrdc_icli_sutil_usr_char_get(handle, _FKEY_WAIT_TIME, &c) ) {
        ...
        case ICLI_KEY_CTRL_('Q'):
            if (!in_multiline_mode) {
                ...
                (void)mrdc_icli_sutil_usr_char_put(handle, 'Q');
                // go to end
                _goto_end( handle );
                ICLI_PUT_NEWLINE;
                _fkey_full_cmd( handle );
...
)
{
    ...
    /* print prompt */
    if ( mrdc_icli_str_len(_MORE_PROMPT) < handle->runtime_data.width - 2 ) {
