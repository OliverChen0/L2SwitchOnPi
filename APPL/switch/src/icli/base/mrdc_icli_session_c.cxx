...
)
{
    ...
    /* update cmd */
    (void)mrdc_icli_str_cpy(handle->runtime_data.cmd + cmd_pos, handle->runtime_data.cmd + handle->runtime_data.cmd_pos);
        ...
        if ( b_end ) {
            if ( cmd_len ) {
                for ( i = handle->runtime_data.cursor_pos; i > 0; --i ) {
                    (void)mrdc_icli_sutil_usr_char_put(handle, ICLI_BACKSPACE);
                }
                for ( i = handle->runtime_data.cursor_pos; i > 0; --i ) {
                    (void)mrdc_icli_sutil_usr_char_put(handle, ICLI_SPACE);
                }
                for ( i = handle->runtime_data.cursor_pos; i > 0; --i ) {
                    (void)mrdc_icli_sutil_usr_char_put(handle, ICLI_BACKSPACE);
                }
