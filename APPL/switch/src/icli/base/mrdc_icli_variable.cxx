...
static icli_variable_data_t     g_variable_data[] = {
    ...
    {
        "ICLI_VARIABLE_LOOPBACK_ID",// name
        "loopback_id",              // variable
        "u32",                      // decl_type
        "0",                        // init_val
        FALSE,                      // b_pointer_type
        FALSE,                      // b_string_type
...
/*
    This should be aligned with icli_url_proto_t in mrdc_icli_type.h
*/
...
#if 0
// Pure function, no side effects
static i32 _ipv4_mask_to_prefix(const mrdc_ipv4_t ipv4, u32 *const prefix)
{
    u32 data = htonl(ipv4);
...
// Pure function, no side effects
static i32 _ipv6_mask_to_prefix(const mrdc_ipv6_t *ipv6, u32 *prefix)
{
...
)
{
    ...
    memcpy(val, &mac_ucast, sizeof(mrdc_mac_t));
    return ICLI_RC_OK;
}
...
)
{
    ...
    memcpy(val, &mac_mcast, sizeof(mrdc_mac_t));
    return ICLI_RC_OK;
}
...
)
{
    ...
    /* Little endian */
    if (val) {
        (*val) = (mrdc_ip_t)( (ip[0] << 24) + (ip[1] << 16) + (ip[2] << 8) + ip[3] );
    }
    return ICLI_RC_OK;
}
...
)
{
    ...
    */
    ...
    if ( u == 0 || u == 255 ) {
        c = word + mrdc_icli_str_len(word) - 1;
        _ERR_MATCH_RETURN();
    }
#endif
...
)
{
    ...
    /* get val */
    memset( val, 0, sizeof(mrdc_ipv6_t) );
...
)
{
    ...
    if (val) {
        memcpy( val, &ipv6, sizeof(mrdc_ipv6_t) );
    }
    return ICLI_RC_OK;
}
...
/*
    ...
    RETURN
        icli_rc_t
*/
static i32 _ipv6_subnet_get(char *word, mrdc_ipv6_network_t *val, u32 *err_pos)
{
    char        *c, *ip_str, *netmask_str;
    mrdc_ipv6_t ip, netmask;
...
/*
    ...
    RETURN
        icli_rc_t
    COMMENT
        the length of val should be at least mrdc_icli_str_len(word)+1, this check
...
)
{
    ...
    if ( mrdc_icli_str_len(word) > ICLI_VALUE_STR_MAX_LEN ) {
        *err_pos = ICLI_VALUE_STR_MAX_LEN;
        return ICLI_RC_ERR_MATCH;
    }

    (void)mrdc_icli_str_cpy(val, word);
    return ICLI_RC_OK;
}
...
)
{
    ...
    /* the copy is done in _node_match(), mrdc_icli_exec.c */
    (void)mrdc_icli_str_cpy(val, word);
#endif
    return ICLI_RC_OK;
}
...
)
{
    ...
    char    *c = word;
    //by type
    i32     len = mrdc_icli_str_len( word );
    ...
    //do not copy ""
    (void)mrdc_icli_str_ncpy(val, word + 1, len - 2);
    val[len - 2] = 0;
    return ICLI_RC_OK;
}
...
{
    u32     len = mrdc_icli_str_len( word );
    ...
    if ( val ) {
        (void)mrdc_icli_str_ncpy(val, word, len);
        val[len] = 0;
    }
...
)
{
    ...
    for (;;) {
        ...
        if ( ICLI_IS_(EOS, *c) ) {
            ...
            if ( mrdc_icli_str_cmp(switch_list, ICLI_INTERFACE_WILDCARD) == 0 ) {
                val->cnt = 1;
        ...
        *deli = ICLI_EOS;
        ...
        /* get switch List */
#if 1 /* CP, 07/18/2013 15:48, Bugzilla#11844 - interface wildcard */
        if ( mrdc_icli_str_cmp(switch_list, ICLI_INTERFACE_WILDCARD) ) {
            r = _uint_range_list_get_in_range( switch_list,
                                               ICLI_DASH,
                                               ICLI_MIN_UINT16,
                                               ICLI_MAX_UINT16,
        ...
        *deli = ICLI_PORT_DELIMITER;
        ...
        /* get port List */
#if 1 /* CP, 07/18/2013 15:48, Bugzilla#11844 - interface wildcard */
        if ( mrdc_icli_str_cmp(port_list, ICLI_INTERFACE_WILDCARD) ) {
...
{
    return _uint_get_in_range(word, 
                               ICLI_MIN_LOOPBACK_ID, 
                               ICLI_MAX_LOOPBACK_ID, 
                               val, 
                               err_pos);
}
...
#ifdef MRDC_ICLI_TOOL
#define __PORT_TYPE_CHECK_AND_ASSIGN(t, x)                              \
    if ( mrdc_icli_str_sub(word, g_port_type_data[t].name, FALSE, &(x)) >= 0 ) { \
        *val = t;                                                       \
        return ICLI_RC_OK;                                              \
    }
#else
#define __PORT_TYPE_CHECK_AND_ASSIGN(t, x) \
    if ( mrdc_icli_port_type_present(t) &&                              \
         mrdc_icli_str_sub(word, g_port_type_data[t].name, FALSE, &(x)) >= 0 ) { \
        *val = t;                                                       \
        return ICLI_RC_OK;                                              \
    }
#endif
...
{
    if ( mrdc_icli_str_sub(word, ICLI_GREP_KEYWORD, 0, err_pos) == -1 ) {
        return ICLI_RC_ERR_MATCH;
    }
...
{
    if ( mrdc_icli_str_sub(word, ICLI_GREP_BEGIN_KEYWORD, 0, err_pos) == -1 ) {
        return ICLI_RC_ERR_MATCH;
    }
...
{
    if ( mrdc_icli_str_sub(word, ICLI_GREP_INCLUDE_KEYWORD, 0, err_pos) == -1 ) {
        return ICLI_RC_ERR_MATCH;
    }
...
{
    if ( mrdc_icli_str_sub(word, ICLI_GREP_EXCLUDE_KEYWORD, 0, err_pos) == -1 ) {
        return ICLI_RC_ERR_MATCH;
    }
...
/*
    ...
    COMMENT
        the length of val should be at least mrdc_icli_str_len(word)+1,
...
)
{
    ...
    if ( mrdc_icli_str_len(word) > ( ICLI_HEXVAL_MAX_LEN + 1 ) ) {
...
)
{
    ...
    // check each char
    for (++c; ICLI_NOT_(EOS, *c); ++c) {
        if (ICLI_NOT_ALPHABET(*c) && ICLI_NOT_DIGIT(*c) && ICLI_NOT_(DASH, *c) && ICLI_NOT_(UNDERLINE, *c)) {
            _ERR_MATCH_RETURN();
        }
    }
...
)
{
    ...
    for ( proto = (icli_url_proto_t)0; proto < ICLI_URL_PROTOCOL_MAX; ++proto ) {
        if ( mrdc_icli_str_cmp(proto_str, g_url_proto_str[proto]) == 0 ) {
            break;
        }
    }
    ...
    // flash:<file>
    if ( proto == ICLI_URL_PROTOCOL_FLASH ) {
        rc = mrdc_icli_file_name_check(c, err_pos);
        if (rc != ICLI_RC_OK) {
            if ((rc == ICLI_RC_ERR_MATCH) && err_pos) {
...
)
{
    ...
    for ( proto = (icli_url_proto_t)0; proto < ICLI_URL_PROTOCOL_MAX; ++proto ) {
        if ( mrdc_icli_str_cmp(s1, g_url_proto_str[proto]) == 0 ) {
            break;
        }
    }
...
_URL_HOST_PORT_GET:
    ...
    if ( ICLI_IS_(COLON, *c) ) {
        ...
        if ( ICLI_IS_(SLASH, *c) || ICLI_IS_(EOS, *c) ) {
            ...
            if ( mrdc_icli_str_len(s2) == 0 ) {
                return ICLI_RC_ERR_INCOMPLETE;
            }

            if ( mrdc_icli_str_len(s2) > ICLI_URL_PORT_MAX_LEN ) {
                _ERR_MATCH_RETURN();
            }
...
)
{
    ...
    rc = mrdc_icli_file_name_check(word, err_pos);
    if (rc != ICLI_RC_OK) {
        return rc;
    }
...
)
{
    ...
    // get protocol
    proto = mrdc_icli_variable_url_protocol_get(word);

    // if flash: then the file already is checked in _url_get()
    if ( proto == ICLI_URL_PROTOCOL_FLASH ) {
        goto _URL_FILE_RC_OK;
    }

    // get file name
    file = word + mrdc_icli_str_len( word ) - 1;
...
)
{
    ...
    if (domain_name_rc == ICLI_RC_OK && val) {
        ...
        (void)mrdc_icli_str_cpy(val->u.u_domain_name, domain_name);
    }
...
#define _WORD_VARIABLE_RANGE_GET( x ) \
    r = mrdc_icli_str_sub(mrdc_icli_variable_data_variable_get( ICLI_VARIABLE_##x ), name, 1, NULL); \
    if ( r == 1 ) { \
        t = ICLI_VARIABLE_RANGE_##x; \
        range_str = name + mrdc_icli_str_len( mrdc_icli_variable_data_variable_get( ICLI_VARIABLE_##x ) ); \
        break; \
    }
...
)
{
    ...
    for ( i = 0; i < ICLI_VARIABLE_KEYWORD; ++i ) {
        if ( mrdc_icli_str_cmp(name, mrdc_icli_variable_data_variable_get((icli_variable_type_t)i)) == 0 ) {
...
)
{
    ...
    switch ( type ) {
        ...
        CASE_VARIABLE( LOOPBACK_ID, loopback_id );  // ICLI_VARIABLE_LOOPBACK_ID
...
/*
    ...
    COMMENT
        n/a
*/
extern "C" char *mrdc_icli_variable_port_type_get_name(
...
)
{
    ...
    for ( p = (icli_url_proto_t)0; p < ICLI_URL_PROTOCOL_MAX; ++p ) {
        if ( mrdc_icli_str_cmp(s, g_url_proto_str[p]) == 0 ) {
            break;
        }
    }
