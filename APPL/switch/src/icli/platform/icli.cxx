...
*/
/* Vitesse trace */
static mrdc_trace_reg_t trace_reg = {
    MRDC_TRACE_MODULE_ID, "icli", "ICLI Engine"
};

static mrdc_trace_grp_t trace_grps[] = {
    [MRDC_TRACE_GRP_DEFAULT] = {
        "default",
        "Default",
        MRDC_TRACE_LVL_WARNING
    }
};

MRDC_TRACE_REGISTER(&trace_reg, trace_grps);
...
)
{
    ...
    /* check history_pos */
    if ( history->history_pos >= mrdc_icli_sutil_history_cmd_cnt(handle) ) {
        return ICLI_RC_ERR_PARAMETER;
    }
    ...
    /* get history */
    (void)mrdc_icli_str_cpy( history->history_cmd, hcmd->cmd );
    return ICLI_RC_OK;
}
...
)
{
    ...
    (void)mrdc_icli_str_cpy(handle->runtime_data.cmd, cmd);
...
/*
    ...
    COMMENT
        n/a
*/
mrdc_rc icli_init(mrdc_init_data_t *data)
{
    icli_init_data_t init_data;
    BOOL             b;
#ifdef MRDC_SW_OPTION_ICFG
    mrdc_rc          rc = MRDC_RC_OK;
#endif
    ...
    case INIT_CMD_EARLY_INIT:
        ...
        if (mrdc_icli_init(&init_data) != ICLI_RC_OK) {
    ...
    case INIT_CMD_CONF_DEF:
        if (data->isid == MRDC_ISID_GLOBAL && !icli_conf_file_load(TRUE)) {
            T_E("Failed to load config");
            return MRDC_RC_ERROR;
        }

        if (MRDC_ISID_LEGAL(data->isid) && !data->switch_info[data->isid].configurable) {
...
)
{
    ...
    memcpy(conf, mrdc_icli_conf_get(), sizeof(icli_conf_data_t));
...
)
{
    ...
    rc = (icli_rc_t)mrdc_icli_conf_default();
...
)
{
    ...
    *input_style = mrdc_icli_input_style_get();
...
)
{
    ...
    rc = (icli_rc_t)mrdc_icli_input_style_set( input_style );
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    cmd_id = mrdc_icli_register_cmd( cmd_register );
    _ICLI_SEMA_GIVE();
    return cmd_id;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_is_enable( cmd_id, enable );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_enable( cmd_id );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_disable( cmd_id );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_is_visible( cmd_id, visible );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_visible( cmd_id );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_invisible( cmd_id );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_privilege_get( cmd_id, privilege );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_register_cmd_privilege_set( cmd_id, privilege );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
/*
    ...
    the continuous service. For example,
        while( 1 ) {
            mrdc_icli_session_engine( session_id );
        }
...
)
{
    ...
    i = mrdc_icli_session_engine( session_id );
...
)
{
    ...
    rc = mrdc_icli_session_open( open_data, session_id );
...
)
{
    ...
    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_session_close( session_id );
    _ICLI_SEMA_GIVE();
    return rc;
}
...
)
{
    ...
    mrdc_icli_session_close_by_username( username );
...
void icli_session_all_close(icli_session_way_t way)
{
    ...
    for ( session_id = 0; session_id < ICLI_SESSION_CNT; ++session_id ) {
        ...
        if (!mrdc_icli_session_alive(session_id) ||
            (handle = mrdc_icli_session_handle_get(session_id)) == NULL ||
            handle->open_data.way != way) {
            continue;
        }

        (void)mrdc_icli_session_close(session_id);
    }
...
u32 icli_session_max_get(void)
{
    ...
    i = mrdc_icli_session_max_cnt_get();
...
)
{
    ...
    rc = mrdc_icli_session_max_cnt_set( max_sessions );
...
)
{
    ...
    (void)mrdc_icli_session_va_str_put(session_id, format, arglist);
...
)
{
    ...
    /* get user string */
    rc = mrdc_icli_session_usr_str_get( session_id, input_type, str, str_size, end_key );
...
)
{
    ...
    rc = mrdc_icli_session_ctrl_c_get( session_id, wait_time );
...
)
{
    ...
    rc = mrdc_icli_session_mode_get( session_id, mode );
...
)
{
    ...
    rc = mrdc_icli_session_mode_enter( session_id, mode );
...
)
{
    ...
    rc = mrdc_icli_session_mode_exit( session_id );
...
)
{
    ...
    rc = mrdc_icli_session_data_get( data );
...
)
{
    ...
    rc = mrdc_icli_session_data_get_next( data );
...
)
{
    ...
    if ( history_size == 0 ) {
        // free all
        n = mrdc_icli_sutil_history_cmd_cnt( handle );
    } else if ( mrdc_icli_sutil_history_cmd_cnt(handle) > history_size ) {
        // keep the recent histories
        n = mrdc_icli_sutil_history_cmd_cnt( handle ) - history_size;
    }
    mrdc_icli_sutil_history_cmd_free_n( handle, n );
...
)
{
    ...
    if ( location == NULL ) {
        ...
        (void)mrdc_icli_str_ncpy(handle->config_data->location, location, ICLI_LOCATION_MAX_LEN);
    }
...
)
{
    ...
    (void)mrdc_icli_str_ncpy(handle->runtime_data.user_name, user_name, ICLI_USERNAME_MAX_LEN);
...
)
{
    ...
    session_id = mrdc_icli_session_self_id();
    if ( session_id >= mrdc_icli_session_max_cnt_get() ) {
        _ICLI_SEMA_GIVE();
        return ICLI_RC_ERR_PARAMETER;
    }
...
)
{
    ...
    if ((handle = mrdc_icli_session_handle_get(session_id)) == NULL) {
...
)
{
    ...
    mrdc_icli_session_debug_cmd_allow_set( handle, b_allow );
...
)
{
    ...
    b_allow = mrdc_icli_session_debug_cmd_allow_get( handle );
...
)
{
    ...
    b_first_time = mrdc_icli_session_debug_cmd_allow_first_time_get( handle );
...
)
{
    ...
    rc = mrdc_icli_session_mode_para_get(session_id, mode, value);
...
)
{
    ...
    rc = mrdc_icli_mode_prompt_set( mode, name );
...
)
{
    ...
    rc = mrdc_icli_dev_name_set( dev_name );
...
)
{
    ...
    s = mrdc_icli_str_cpy(dev_name, mrdc_icli_dev_name_get());
...
)
{
    ...
    (void)mrdc_icli_str_cpy(banner_login, mrdc_icli_banner_login_get());
...
)
{
    ...
    rc = mrdc_icli_banner_login_set( banner_login );
...
)
{
    ...
    (void)mrdc_icli_str_cpy(banner_motd, mrdc_icli_banner_motd_get());
...
)
{
    ...
    rc = mrdc_icli_banner_motd_set( banner_motd );
...
)
{
    ...
    (void)mrdc_icli_str_cpy(banner_exec, mrdc_icli_banner_exec_get());
...
)
{
    ...
    rc = mrdc_icli_banner_exec_set( banner_exec );
...
)
{
    ...
    b = mrdc_icli_port_type_present( type );
...
{
    return mrdc_icli_variable_port_type_get_name((icli_port_type_t)type);
}
...
{
    return mrdc_icli_variable_port_type_get_short_name(type);
}
...
)
{
    ...
    b = mrdc_icli_port_range_get( range );
...
)
{
    ...
    b = mrdc_icli_port_range_set(range);
...
)
{
    ...
    b = mrdc_icli_port_from_usid_uport( switch_range );
...
)
{
    ...
    b = mrdc_icli_port_from_isid_iport( switch_range );
...
)
{
    ...
    b = mrdc_icli_port_get_first( switch_range );
...
)
{
    ...
    b = mrdc_icli_port_get( switch_range );
...
)
{
    ...
    b = mrdc_icli_port_get_next( switch_range );
...
)
{
    ...
    b = mrdc_icli_port_switch_range_valid( switch_port, switch_id, port_id );
...
)
{
    ...
    b = mrdc_icli_switch_get( switch_range );
...
)
{
    ...
    b = mrdc_icli_switch_get_next( switch_range );
...
)
{
    ...
    b = mrdc_icli_switch_port_get( switch_range );
...
)
{
    ...
    b = mrdc_icli_switch_port_get_next( switch_range );
...
)
{
    ...
    b = mrdc_icli_enable_password_get(priv, password);
...
)
{
    ...
    b = mrdc_icli_enable_password_set(priv, password);
...
)
{
    ...
    b = mrdc_icli_enable_password_verify(priv, clear_password);
...
)
{
    ...
    b = mrdc_icli_enable_secret_set(priv, secret);
...
)
{
    ...
    b = mrdc_icli_enable_secret_set_clear(priv, clear_password);
...
)
{
    ...
    b = mrdc_icli_enable_password_if_secret_get( priv );
...
{
    char   temp_prompt[sizeof(g_prompt)], *s_in, *s_out, s_date_and_time[APPL_RFC3339_TIME_STR_LEN];
    BOOL   overflow = FALSE;
...
)
{
    ...
    rc = mrdc_icli_priv_get_first( conf );
...
)
{
    ...
    rc = mrdc_icli_priv_get_next( conf );
...
mrdc_rc icli_multiline_data_get(u32 session_id, icli_session_multiline_data_t *const data)
{
    mrdc_rc rc = MRDC_RC_OK;

    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_session_multiline_data_get(session_id, data);
    _ICLI_SEMA_GIVE();

    return rc;
}

mrdc_rc icli_multiline_data_set(u32 session_id, const icli_session_multiline_data_t *data)
{
    mrdc_rc rc = MRDC_RC_OK;

    _ICLI_SEMA_TAKE();
    rc = mrdc_icli_session_multiline_data_set(session_id, data);
    _ICLI_SEMA_GIVE();
...
BOOL icli_has_url_with_proto(u32 session_id, icli_url_proto_t req_proto)
{
    ...
    for (p = handle->runtime_data.cmd_var; p; ___NEXT(p)) {
        ...
        if (!mrdc_icli_exec_para_word_get(p, temp_buf)) {
            continue;
        }

        // check if URL has requested protocol
        icli_url_proto_t proto = mrdc_icli_variable_url_protocol_get(temp_buf);
...
bool check_valid_ipv4_ucast_value(mrdc_ipv4_t ipv4)
{
    u8 u = (u8)((ipv4 & 0xff000000) >> 24);
...
bool check_valid_ipv6_ucast_value(mrdc_ipv6_t ipv6)
{
    if (ipv6.addr[0] == 0xFF) {
        return false;
    }
...
bool check_valid_subnet_mask(uint32_t mask) {
