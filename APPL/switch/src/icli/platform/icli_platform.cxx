...
#define _CSI \
    (void)mrdc_icli_sutil_usr_char_put(handle, ICLI_KEY_ESC); \
    (void)mrdc_icli_sutil_usr_char_put(handle, _SEQUENCE_INDICATOR);
...
static const icli_cmd_mode_info_t g_cmd_mode_info[ICLI_CMD_MODE_MAX] = {
    ...
    {
        ICLI_CMD_MODE_INTERFACE_LOOPBACK,   // mode
        "ICLI_CMD_MODE_INTERFACE_LOOPBACK", // mode str
        "loopback Interface Mode",          // description
        "config-if-lo",                     // prompt
        "if-looback",                       // name
        "interface looback <looback_id>",   // ICLI command
        TRUE                                // b_mode_var
    },
    {
    ...
    {
        ICLI_CMD_MODE_TSN_PSFP_STREAM_FILTER,   // mode
        "ICLI_CMD_MODE_TSN_PSFP_STREAM_FILTER", // mode str
    ...
    {
        ICLI_CMD_MODE_TSN_PSFP_STREAM_GATE,   // mode
        "ICLI_CMD_MODE_TSN_PSFP_STREAM_GATE", // mode str
...
)
{
    ...
    for ( mode = (icli_cmd_mode_t)0; mode < ICLI_CMD_MODE_MAX; ++mode ) {
        if ( mrdc_icli_str_cmp(mode_str, g_cmd_mode_info[mode].str) == 0 ) {
            break;
        }
    }
...
)
{
    ...
    if ( mrdc_icli_str_to_int(priv_str, &priv) == 0 ) {
    ...
    for ( priv = 0; priv < ICLI_PRIVILEGE_MAX; ++priv ) {
        if ( mrdc_icli_str_cmp(priv_str, g_privilege_str[priv]) == 0 ) {
            break;
        }
    }
    return (icli_privilege_t)priv;
...
{
    _CSI;
    (void)mrdc_icli_sutil_usr_char_put(handle, _CURSOR_UP);
}
...
{
    _CSI;
    (void)mrdc_icli_sutil_usr_char_put(handle, _CURSOR_DOWN);
}
...
)
{
    ...
    // get current x,y
    mrdc_icli_sutil_current_xy_get(handle, &x, &y);
    ...
    if ( x < max_x ) {
        // just go forward
        _CSI;
        (void)mrdc_icli_sutil_usr_char_put(handle, _CURSOR_FORWARD);
    } else {
...
)
{
    ...
    if ( x ) {
        // just backward
        _CSI;
        (void)mrdc_icli_sutil_usr_char_put(handle, _CURSOR_BACKWARD);
    } else {
...
)
{
    ...
    if ( offset_x > 0 ) {
        ...
        mrdc_icli_session_str_put(handle, s);
        (void)mrdc_icli_sutil_usr_char_put(handle, _CURSOR_FORWARD);
    } else if ( offset_x < 0 ) {
        // backward
        _CSI;
...
)
{
    ...
    case ICLI_SESSION_WAY_CONSOLE:
        agent = MRDC_APPL_AUTH_AGENT_CONSOLE;
        break;

    case ICLI_SESSION_WAY_TELNET:
        agent = MRDC_APPL_AUTH_AGENT_TELNET;
        break;

    case ICLI_SESSION_WAY_SSH:
        agent = MRDC_APPL_AUTH_AGENT_SSH;
        break;
...
)
{
    ...
    (void)mrdc_icli_str_cpy(user_name, handle->runtime_data.user_name);
...
_icli_cmd_authorize_end:
    ...
    mrdc_icli_exec_full_cmd_str_get_end(handle);
    return rc;
}
...
{
    return (u16)( topo_isid2usid((mrdc_isid_t)isid) );
}
...
{
    return (u16)( iport2uport((mrdc_port_no_t)iport) );
}
...
)
{
    ...
    if ((rc = switch_iter_init(&s_iter, MRDC_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_USID_CFG)) != MRDC_RC_OK) {
    ...
    while (switch_iter_getnext(&s_iter)) {
        u32 cpu_cnt = 0, begin = 0, port_cnt = 0;
        if ((rc = port_iter_init(&p_iter, NULL, s_iter.isid, PORT_ITER_SORT_ORDER_UPORT, PORT_ITER_FLAGS_FRONT | PORT_ITER_FLAGS_CPU)) != MRDC_RC_OK) {
        ...
        while (port_iter_getnext(&p_iter)) {
            ...
            MRDC_RC_ERR_PRINT(port_cap_get(p_iter.iport, &cap));
            if (cap & (MEBA_PORT_CAP_25G_FDX)) {
                ...
                // RACOON设备端口从0开始显示，则begin的开始值为0
                begin--;
            #endif
            
            T_I("r_idx: %d, port_cnt:%d, begin:%d, isid: %d, iport: %d, uport: %d", 
                r_idx, rng->switch_range[r_idx].port_cnt, begin, s_iter.isid, p_iter.iport, p_iter.uport);
    ...
    rng->cnt = r_idx;
    b_rc = mrdc_icli_port_range_set(rng);
    icli_free(rng);
...
)
{
    ...
    for ( mode = ICLI_CMD_MODE_EXEC; mode < ICLI_CMD_MODE_MAX; ++mode ) {
        if ( mrdc_icli_str_sub(name, g_cmd_mode_info[mode].name, 0, NULL) != -1 ) {
            return mode;
        }
    }
