...
BOOL mrdc_icli_get_first_port(port_iter_t  *pit)
{
    if (!pit) {
        return false;
    }
...
BOOL mrdc_icli_get_next_port(port_iter_t  *pit)
{
    if (!pit) {
        return FALSE;
    }
    ...
    case PORT_ITER_STATE_FIRST:
        return mrdc_icli_get_first_port(pit);
    default:
...
static char *icli_port_info_txt_generic(mrdc_usid_t usid, mrdc_port_no_t uport, char *str_buf_p, BOOL is_short)
{
...
{
    if (usid == MRDC_USID_LOCAL) {
        usid = topo_isid2usid(MRDC_ISID_START);
    }
...
int icli_porttypelist2iportlist(mrdc_usid_t usid, icli_stack_port_range_t *port_type_list_p, mrdc_port_list_t &port_list)
{
    ...
    for (range_idx = 0; range_idx < port_type_list_p->cnt; range_idx++) {
        if (usid != MRDC_ISID_GLOBAL && usid != port_type_list_p->switch_range[range_idx].usid) {
...
{
    return switch_iter_init(sit, MRDC_ISID_GLOBAL, SWITCH_ITER_SORT_ORDER_USID_CFG);
}
...
BOOL icli_port_iter_getnext(port_iter_t *pit, icli_stack_port_range_t *list)
{
    ...
    while (mrdc_icli_get_next_port(pit)) {
                           ...
                           icli_runtime_t     *runtime)
{
    ...
    case ICLI_ASK_BYWORD:
        // Subtract 1, because the last switch is the MRDC_ISID_GLOBAL which shall not  be used here.
        icli_sprintf(runtime->byword, "<Switch ID: %u-%u>", MRDC_USID_START, MRDC_USID_END - 1);
        return TRUE;
    case ICLI_ASK_RANGE:
        runtime->range.type = ICLI_RANGE_TYPE_UNSIGNED;
        runtime->range.u.sr.cnt = 1;
        runtime->range.u.sr.range[0].min = MRDC_USID_START;
        runtime->range.u.sr.range[0].max = MRDC_USID_END - 1; // Subtract 1, because the last switch is the MRDC_ISID_GLOBAL which shall not  be used here.
        return TRUE;
    case ICLI_ASK_HELP:
...
{
    mrdc_usid_t isid = topo_usid2isid(usid);
...
BOOL icli_cmd_switch_range_exist(u32 session_id, icli_unsigned_range_t *switch_list_p, BOOL omit_non_exist, BOOL alert_configurable_but_non_existing)
{
    ...
    for (range_idx = 0; range_idx < switch_list_p->cnt; range_idx++) {
        ...
        for (usid_idx = (mrdc_usid_t)switch_list_p->range[range_idx].min;
             usid_idx <= (mrdc_usid_t)switch_list_p->range[range_idx].max; usid_idx++) {
            ...
            if (omit_non_exist &&
                !is_new_range &&
                (is_non_existing || (usid_idx == (mrdc_usid_t)switch_list_p->range[range_idx].max /* last new range */))) {
                                ...
                                icli_runtime_t     *runtime)
{
    ...
    case ICLI_ASK_PRESENT:
        ...
        MRDC_RC(icli_port_iter_init(&pit, MRDC_ISID_START, PORT_ITER_FLAGS_ALL));
                             ...
                             icli_runtime_t     *runtime)
{
    ...
    case ICLI_ASK_PRESENT:
        ...
        MRDC_RC(icli_port_iter_init(&pit, MRDC_ISID_START, PORT_ITER_FLAGS_CPU));
        while (icli_port_iter_getnext(&pit, plist)) {
            if (ICLI_PORT_TYPE_CPU == pit.port_type) {
                runtime->present = TRUE;
                return TRUE;
