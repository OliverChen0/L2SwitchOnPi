...
static mrdc_handle_t ssh_handle;
...
)
{
    ...
    if (mgmt_txt2ipv4(sshconn, &open_data.client_ip.addr.ipv4, NULL, FALSE) == MRDC_RC_OK) {
        T_I("ssh IPv4: %s", sshconn);
        open_data.client_ip.type = MRDC_IP_TYPE_IPV4;
    }
#ifdef MRDC_SW_OPTION_IPV6
    else if (mgmt_txt2ipv6(sshconn, &open_data.client_ip.addr.ipv6) == MRDC_RC_OK) {
        T_I("ssh IPv6: %s", sshconn);
        open_data.client_ip.type = MRDC_IP_TYPE_IPV6;
    }
#endif
    else {
        T_E("ssh address error: %s", sshconn);
        open_data.client_ip.type = MRDC_IP_TYPE_NONE;
    }
...
)
{
    ...
    BOOL                b_loop;
    i32                 i;
    const char *path = MRDC_FS_RUN_DIR "icli.socket";
...
)
{
    ...
    mrdc_thread_create(MRDC_THREAD_PRIO_DEFAULT,
                       _stubs_thread,
                       0,
                       "ICLI SSH Server",
                       nullptr,
