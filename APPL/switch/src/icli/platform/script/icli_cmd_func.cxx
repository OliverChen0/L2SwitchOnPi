...
)
{
    ...
    len = mrdc_icli_str_len(user_str) - 1;
    if ( len >= max_len ) {
    ...
    *(c + 1) = 0;
    ...
    while ( b_loop ) {
        ...
        case ICLI_RC_OK:
            ...
            // check length
            // -1 is for the starting delimiter
            len = mrdc_icli_str_len(user_str) - 1 + mrdc_icli_str_len(str);
            if ( len >= max_len ) {
                if ( mrdc_icli_str_len(user_str) ) {
                    len = max_len - mrdc_icli_str_len(user_str) + 1;
                } else {
                    len = max_len;
                }
                *(str + len) = 0;
                (void)mrdc_icli_str_concat(user_str, str);
                // free memory
                icli_free(str);
                return TRUE;
            }
            ...
            // prepare for next input
            len = mrdc_icli_str_len(user_str);
            user_str[len]   = '\n';
            user_str[len + 1] = 0;
            break;
