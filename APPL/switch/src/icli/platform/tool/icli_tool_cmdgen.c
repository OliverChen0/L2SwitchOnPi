...
#define ___VALUE_STR_GET(p) \
    p = (char *)icli_malloc(mrdc_icli_str_len(g_value_str)+1);\
    if ( p == NULL ) {\
        T_E("icli_malloc() for "#p" by length %u\n", mrdc_icli_str_len(g_value_str)+1);\
        return ICLI_RC_ERROR;\
    }\
    (void)mrdc_icli_str_cpy(p, g_value_str);

#define ___TAG_GET(p) \
    p = (char *)icli_malloc(mrdc_icli_str_len(g_tag_str)+1);\
    if ( p == NULL ) {\
        T_E("icli_malloc() for "#p" by length %u\n", mrdc_icli_str_len(g_tag_str)+1);\
        return ICLI_RC_ERROR;\
    }\
    (void)mrdc_icli_str_cpy(p, g_tag_str);

#define ___VALUE_STR_GET_WIHT_EXTRA_LEN(p, len) \
    p = (char *)icli_malloc(mrdc_icli_str_len(g_value_str)+len+1);\
    if ( p == NULL ) {\
        T_E("icli_malloc() for "#p" by length %u\n", mrdc_icli_str_len(g_value_str)+len+1);\
        return ICLI_RC_ERROR;\
    }\
    (void)mrdc_icli_str_cpy(p, g_value_str);

#define ___LINE_BUF_GET(p) \
    p = (char *)icli_malloc(mrdc_icli_str_len(g_line_buf)+1);\
    if ( p == NULL ) {\
        T_E("icli_malloc() for "#p" by length %u\n", mrdc_icli_str_len(g_line_buf)+1);\
        return ICLI_RC_ERROR;\
    }\
    (void)mrdc_icli_str_cpy(p, g_line_buf);
...
static _tag_t _tag_get_by_str(char *tag_str)
{
    ...
    for ( i = 0; i < _TAG_MAX; ++i ) {
        if (mrdc_icli_str_cmp(tag_str, g_tag_name[i]) == 0) {
            return i;
        }
    }
    return i;
...
static i32 _more_value_str_get( void )
{
    ...
    j = mrdc_icli_str_len( g_value_str );
...
static i32 _add_str_def( void )
{
    ...
    if ( mrdc_icli_str_len(g_value_str) == 0 ) {
...
static i32 _value_str_get( void )
{
    ...
    case _TAG_MODULE_IF_FLAG:
        ...
        */
    case _TAG_COMMAND:
        ___VALUE_STR_MALLOC_AND_GET_WITH_EXTRA_LEN(g_current_cmd->command, TRUE, mrdc_icli_str_len(___GREP_CMD_STR));
        break;
...
#define ___MODULE_WITH_VALUE_STR(p)\
    ( g_module->p && mrdc_icli_str_len(g_module->p->value_str) )

#define ___CMD_NO_VALUE_STR(p)\
    ( g_current_cmd->p == NULL || mrdc_icli_str_len(g_current_cmd->p->value_str) == 0 )

#define ___CMD_WITH_VALUE_STR(p)\
    ( g_current_cmd->p && mrdc_icli_str_len(g_current_cmd->p->value_str) )
...
#define ___tag_CMD_EXPAND(tag_list) \
    ...
    for ( i = 0; i < n_or; ++i ) {\
        for ( j=0, t = t_repeat; j < repeat_cnt; ++j, ___NEXT(t) ) {\
            len = mrdc_icli_str_len(t->value_str);\
            str_len = len + 1 + 2;\
            ___tag_ALLOC(t_new, str_len);\
            if ( len ) {\
                (void)mrdc_icli_str_cpy(t_new->value_str, t->value_str);\
...
static i32 _repeat_process( void )
{
    ...
    // process repeat "..."
    l = mrdc_icli_str_str(_REPEAT_SYNTAX, ___VALUE_STR(command));
    ...
    // revise command
    len = mrdc_icli_str_len(v) * (n_or + 1) + 1;
    cmd = icli_malloc(len);
    if ( cmd == NULL ) {
        T_E("icli_malloc()\n");
        return ICLI_RC_ERROR;
    ...
    if ( b_optional ) {
        (void)mrdc_icli_str_ncpy(cmd, v, (u32)(out_b - v + 1));
        c = cmd + mrdc_icli_str_len(cmd);
        *c++ = ICLI_SPACE;
        ...
        for ( i = 0; i <= n_or; ++i ) {
            ...
            *c++ = ICLI_SPACE;
            ...
            (void)mrdc_icli_str_ncpy(c, in_b + 1, (u32)(in_t - in_b - 1));
            c += mrdc_icli_str_len(c);
        ...
        *c++ = ICLI_SPACE;
        (void)mrdc_icli_str_cpy(c, out_t);
    } else {
        (void)mrdc_icli_str_ncpy(cmd, v, (u32)(l - v));
        c = cmd + mrdc_icli_str_len(cmd);
        *c++ = ICLI_SPACE;

        for ( i = 0; i < n_or; ++i ) {
            (void)mrdc_icli_str_ncpy(c, out_b + 1, (u32)(l - out_b - 1));
            c += mrdc_icli_str_len(c);
            *c++ = ICLI_SPACE;
        }
    ...
    // get w_cnt
    // -1 because of repeat "..."
    w_cnt = mrdc_icli_word_count( v ) - 1;
    if ( w_cnt < 0 ) {
    ...
    *l     = ICLI_EOS;
    ...
    pre_cnt  = mrdc_icli_word_count( v );
    if ( pre_cnt < 0 ) {
        T_E("fail to get command word count\n");
        return ICLI_RC_ERROR;
    }
    repeat_cnt = mrdc_icli_word_count( out_b + 1 );
    if ( repeat_cnt < 0 ) {
        T_E("fail to get command word count\n");
        return ICLI_RC_ERROR;
    }
...
static i32 _port_type_id_process( void )
{
    ...
    p = mrdc_icli_str_str(_PORT_TYPE_ID, ___VALUE_STR(command));
    if ( p == NULL ) {
        return ICLI_RC_OK;
    }
    ...
    */
    ...
    // 2 for two spaces
    len = mrdc_icli_str_len(v) + mrdc_icli_str_len(_PORT_TYPE) + 16 + 1;
    cmd = icli_malloc(len);
    if ( cmd == NULL ) {
        T_E("icli_malloc()\n");
        return ICLI_RC_ERROR;
    ...
    *c = ' ';
    ...
    // add _PORT_TYPE
    (void)mrdc_icli_str_cpy(c + 1, _PORT_TYPE);
    ...
    */
    ...
    if ( t && t->next ) {
        // allocate new one
        ___tag_ALLOC(new_tag, mrdc_icli_str_len(t->next->value_str) + 1);
        // copy value
        new_tag->line = t->next->line;
        (void)mrdc_icli_str_cpy(new_tag->value_str, t->next->value_str);
        // chain
        new_tag->next = t->next;
        t->next = new_tag;
    }
    ...
    */
    ...
    if ( t && t->next ) {
        // allocate new one
        ___tag_ALLOC(new_tag, mrdc_icli_str_len(_HELP_PORT_TYPE) + 1);
        // help for port type
        new_tag->line = t->next->line;
        (void)mrdc_icli_str_cpy(new_tag->value_str, _HELP_PORT_TYPE);
    ...
    if ( p ) {
        ...
        for ( p = mrdc_icli_str_str(_XORT_TYPE_ID, p); p; p = mrdc_icli_str_str(_XORT_TYPE_ID, p + 1) ) {
            *(p + 1) = 'p';
        }
        // get word count
        g_current_cmd->command_word_cnt = mrdc_icli_word_count( ___VALUE_STR(command) );
...
static i32 _port_type_list_process( void )
{
    ...
    /* check if exist or not */
    p = mrdc_icli_str_str(_PORT_TYPE_LIST, ___VALUE_STR(command));
    if ( p == NULL ) {
        return ICLI_RC_OK;
    }
    ...
    *c = ' ';
    ...
    // add _XORT_TYPE_LIST
    (void)mrdc_icli_str_cpy(c + 1, _XORT_TYPE_LIST);
    ...
    *c = ' ';
    ...
    // copy after
    p += mrdc_icli_str_len(_PORT_TYPE_LIST);
    (void)mrdc_icli_str_cpy(c + 1, p);
    ...
    */
    ...
    if ( p ) {
        ...
        p = ___VALUE_STR(command);
        for ( p = mrdc_icli_str_str(_XORT_TYPE_LIST, p); p; p = mrdc_icli_str_str(_XORT_TYPE_LIST, p + 1) ) {
            *(p + 1) = 'p';
        }
    }
    return rc;
...
static i32 _dscp_process( void )
{
    ...
    icli_dscp_wvh_t *dscp_wvh = mrdc_icli_variable_dscp_wvh_get();

    /* check if exist or not */
    p = mrdc_icli_str_str(_DSCP_VAR, ___VALUE_STR(command));
    if ( p == NULL ) {
        return ICLI_RC_OK;
    }
    ...
    */
    ...
    /* get new command buffer */
    len = mrdc_icli_str_len(v) + mrdc_icli_str_len(_DSCP_EXPAND) + 8;
    cmd = icli_malloc(len);
    if ( cmd == NULL ) {
        T_E("icli_malloc()\n");
        return ICLI_RC_ERROR;
    ...
    *c = ' ';
    ...
    // add _DSCP_EXPAND
    (void)mrdc_icli_str_cpy(c + 1, _DSCP_EXPAND);
    ...
    */
    ...
    if ( t ) {
        len = mrdc_icli_str_len(t->value_str) + 1;
        for ( j = 1; j < ICLI_DSCP_MAX_CNT; ++j ) {
            // allocate new one
            ___tag_ALLOC(new_tag, len);
            // copy
            new_tag->line = t->line;
            (void)mrdc_icli_str_cpy(new_tag->value_str, t->value_str);
            // chain
            new_tag->next = t->next;
            t->next = new_tag;
            t = new_tag;
    ...
    */
    ...
    j = 0;
    c = (char *)icli_malloc(mrdc_icli_str_len(dscp_wvh[j].help) + 1);
    if ( c == NULL ) {
        T_E("icli_malloc()\n");
        return ICLI_RC_ERROR;
    }
    (void)mrdc_icli_str_cpy(c, dscp_wvh[j].help);
    icli_free(t->value_str);
    t->value_str = c;

    for ( j = 1; j < ICLI_DSCP_MAX_CNT; ++j ) {
        // allocate new one
        ___tag_ALLOC(new_tag, mrdc_icli_str_len(dscp_wvh[j].help) + 1);
        // copy
        new_tag->line = t->line;
        (void)mrdc_icli_str_cpy(new_tag->value_str, dscp_wvh[j].help);
        // chain
        new_tag->next = t->next;
        t->next = new_tag;
        t = new_tag;
...
static i32 _no_and_default_form_add( void )
{
    ...
    if ( g_current_cmd->no_form_code ) {
        ...
        *new_command = *g_current_cmd;
        // original_cmd
        len = mrdc_icli_str_len(g_current_cmd->original_cmd) + mrdc_icli_str_len(_NO_FORM_PREFIX) + 1;
        new_command->original_cmd = (char *)icli_malloc(len);
        memset(new_command->original_cmd, 0, len);
        (void)mrdc_icli_str_cpy(new_command->original_cmd, _NO_FORM_PREFIX);
        (void)mrdc_icli_str_cpy(new_command->original_cmd + mrdc_icli_str_len(_NO_FORM_PREFIX), g_current_cmd->original_cmd);
        // command_word_cnt
        ...
        // command
        len = mrdc_icli_str_len(___VALUE_STR(command)) + mrdc_icli_str_len(_NO_FORM_PREFIX) + 1;
        ___tag_ALLOC(new_command->command, len);
        (void)mrdc_icli_str_cpy(new_command->command->value_str, _NO_FORM_PREFIX);
        (void)mrdc_icli_str_cpy(new_command->command->value_str + mrdc_icli_str_len(_NO_FORM_PREFIX), ___VALUE_STR(command));
        // doc_cmd_desc
        ...
        if ___CMD_WITH_VALUE_STR( func_name ) {
            len = mrdc_icli_str_len(___VALUE_STR(func_name)) + mrdc_icli_str_len(_NO_FORM_POSTFIX) + 1;
            ___tag_ALLOC(new_command->func_name, len);
            (void)mrdc_icli_str_cpy(new_command->func_name->value_str, ___VALUE_STR(func_name));
            (void)mrdc_icli_str_cpy(new_command->func_name->value_str + mrdc_icli_str_len(___VALUE_STR(func_name)), _NO_FORM_POSTFIX);
        }
        // func_reuse
        if ___CMD_WITH_VALUE_STR( func_reuse ) {
            len = mrdc_icli_str_len(___VALUE_STR(func_reuse)) + mrdc_icli_str_len(_NO_FORM_POSTFIX) + 1;
            ___tag_ALLOC(new_command->func_reuse, len);
            (void)mrdc_icli_str_cpy(new_command->func_reuse->value_str, ___VALUE_STR(func_reuse));
            (void)mrdc_icli_str_cpy(new_command->func_reuse->value_str + mrdc_icli_str_len(___VALUE_STR(func_reuse)), _NO_FORM_POSTFIX);
        }
        // cmd_var
        ...
        // help
        len = mrdc_icli_str_len(_NO_FORM_HELP) + 1;
        ___tag_ALLOC(new_command->help, len);
        (void)mrdc_icli_str_cpy(new_command->help->value_str, _NO_FORM_HELP);
    ...
    if ( g_current_cmd->default_form_code ) {
        ...
        *new_command = *g_current_cmd;
        // original_cmd
        len = mrdc_icli_str_len(g_current_cmd->original_cmd) + mrdc_icli_str_len(_DEFAULT_FORM_PREFIX) + 1;
        new_command->original_cmd = (char *)icli_malloc(len);
        memset(new_command->original_cmd, 0, len);
        (void)mrdc_icli_str_cpy(new_command->original_cmd, _DEFAULT_FORM_PREFIX);
        (void)mrdc_icli_str_cpy(new_command->original_cmd + mrdc_icli_str_len(_DEFAULT_FORM_PREFIX), g_current_cmd->original_cmd);
        // command_word_cnt
        ++( new_command->command_word_cnt );
        if ( new_command->command_word_cnt > _WORD_MAX_CNT ) {
            __U_E("too many words\n");
        ...
        // command
        len = mrdc_icli_str_len(___VALUE_STR(command)) + mrdc_icli_str_len(_DEFAULT_FORM_PREFIX) + 1;
        ___tag_ALLOC(new_command->command, len);
        (void)mrdc_icli_str_cpy(new_command->command->value_str, _DEFAULT_FORM_PREFIX);
        (void)mrdc_icli_str_cpy(new_command->command->value_str + mrdc_icli_str_len(_DEFAULT_FORM_PREFIX), ___VALUE_STR(command));
        // doc_cmd_desc
        ...
        // func_name
        if ___CMD_WITH_VALUE_STR( func_name ) {
            len = mrdc_icli_str_len(___VALUE_STR(func_name)) + mrdc_icli_str_len(_DEFAULT_FORM_POSTFIX) + 1;
            ___tag_ALLOC(new_command->func_name, len);
            (void)mrdc_icli_str_cpy(new_command->func_name->value_str, ___VALUE_STR(func_name));
            (void)mrdc_icli_str_cpy(new_command->func_name->value_str + mrdc_icli_str_len(___VALUE_STR(func_name)), _DEFAULT_FORM_POSTFIX);
        }
        // func_reuse
        if ___CMD_WITH_VALUE_STR( func_reuse ) {
            len = mrdc_icli_str_len(___VALUE_STR(func_reuse)) + mrdc_icli_str_len(_DEFAULT_FORM_POSTFIX) + 1;
            ___tag_ALLOC(new_command->func_reuse, len);
            (void)mrdc_icli_str_cpy(new_command->func_reuse->value_str, ___VALUE_STR(func_reuse));
            (void)mrdc_icli_str_cpy(new_command->func_reuse->value_str + mrdc_icli_str_len(___VALUE_STR(func_reuse)), _DEFAULT_FORM_POSTFIX);
        }
        // cmd_var
        ___tag_ALLOC(new_command->cmd_var, 1);
        new_command->cmd_var->next = g_current_cmd->cmd_var;
        ...
        len = mrdc_icli_str_len(_NO_FORM_HELP) + 1;
        ___tag_ALLOC(new_command->help, len);
        (void)mrdc_icli_str_cpy(new_command->help->value_str, _DEFAULT_FORM_HELP);
        new_command->help->next = g_current_cmd->help;
        // property
        new_command->property = g_current_cmd->property;
        // variable
    ...
    while ( g_current_cmd != NULL ) {
        ...
        for ( t = g_current_cmd->cmd_mode->next; t != NULL; ___NEXT(t), ++n ) {
            ...
            *new_command = *cmd;
            ...
            if ___CMD_WITH_VALUE_STR( func_name ) {
                len = mrdc_icli_str_len(___VALUE_STR(func_name)) + 8;
                ___tag_ALLOC(new_command->func_name, len);
                (void)mrdc_icli_str_cpy(new_command->func_name->value_str, ___VALUE_STR(func_name));
                icli_sprintf(new_command->func_name->value_str + mrdc_icli_str_len(___VALUE_STR(func_name)), "_%u", n);
            }
            ...
            if ( new_command->mode_info->b_mode_var ) {
                new_command->mode_word_cnt = mrdc_icli_word_count( (char *)(new_command->mode_info->cmd) );
    ...
    for ( sub_mode = ICLI_CMD_MODE_CONFIG_VLAN; sub_mode < ICLI_CMD_MODE_MAX; ++sub_mode ) {
        ...
        *new_command = *g_current_cmd;
        ...
        if ___CMD_WITH_VALUE_STR( property ) {
            if ( mrdc_icli_str_str("ICLI_CMD_PROP_INVISIBLE", ___VALUE_STR(property)) == NULL ) {
            ...
            mrdc_icli_str_cpy( g_value_str, "ICLI_CMD_PROP_INVISIBLE" );
            b_new = TRUE;
        }
        if ( b_new ) {
        ...
        mrdc_icli_str_cpy( g_value_str, mode_info->str );
...
static i32 _sub_mode_def_cmd_add( void )
{
    ...
    *new_command = *command;
    ...
    len = mrdc_icli_str_len( new_command->func_name->value_str ) + 8;
    ___tag_ALLOC( new_command->func_name, len );
...
static i32 _sub_mode_gen( void )
{
    ...
    while ( g_current_cmd ) {
        ...
        // check if sub mode
        if ( ___CMD_NO_VALUE_STR(mode_sub) || mrdc_icli_str_cmp(___VALUE_STR(mode_sub), "1") ) {
...
#if 1 /* CP, 09/30/2013 16:44, Bugzilla#12878 - ICLI presents symbolic constants from COMMANDs to the user */
/*
    _first_word_get() is copied from mrdc_icli_parsing.c
*/
#if ICLI_LOOP_SYNTAX
...
)
{
    ...
    if ( cmd->grep ) {
        (void)mrdc_icli_str_ncpy( g_value_str, c, mrdc_icli_str_len(c) - mrdc_icli_str_len(___GREP_CMD_STR) );
    } else {
        (void)mrdc_icli_str_cpy( g_value_str, c );
    }
    ...
    for ( t = _first_word_get(&w, &c); t != ICLI_WORD_TYPE_EOS; t = _first_word_get(&w, &c) ) {
        ...
        case ICLI_WORD_TYPE_VARIABLE:
            if ( cmd_var && mrdc_icli_str_len(cmd_var->value_str) ) {
...
static i32 _var_cmd_gen( void )
{
    ...
    for ( cmd = g_command; cmd; ___NEXT(cmd) ) {
        ...
        len = mrdc_icli_str_len( g_line_buf ) + 1;
        cmd->var_cmd = (char *)icli_malloc( len );
        memset( cmd->var_cmd, 0, len );
        (void)mrdc_icli_str_cpy( cmd->var_cmd, g_line_buf );
    }
    return ICLI_RC_OK;
}
#endif
            ...
            case _SEGMENT_BEGIN:
                ...
                case _TAG_CMD_END:
                    ...
                    if ___CMD_NO_VALUE_STR( func_name ) {
                        ___tag_ALLOC(g_current_cmd->func_name, mrdc_icli_str_len(g_script_file) + 32);
                    ...
                    r = mrdc_icli_str_len( ___VALUE_STR(command) ) * 2 + 1;
                    ...
                    r = mrdc_icli_str_len(g_current_cmd->original_cmd) + 1;
                    ...
                    memset(c, 0, r);
                    (void)mrdc_icli_str_cpy(c, g_current_cmd->original_cmd);
                    ...
                    if ___CMD_WITH_VALUE_STR( property ) {
                        if (mrdc_icli_str_str("ICLI_CMD_PROP_GREP", g_current_cmd->property->value_str) != NULL) {
                            g_current_cmd->grep = 1;
                            // realloc memory for grep string
                            c = (char *)icli_malloc(mrdc_icli_str_len(___VALUE_STR(command)) + mrdc_icli_str_len(___GREP_CMD_STR) + 1);
                            if ( c == NULL ) {
                                T_E("memory insufficient\n");
                                return ICLI_RC_ERROR;
                            }
                            // copy original cmd string
                            (void)mrdc_icli_str_cpy(c, ___VALUE_STR(command));
                            // free original string
                            icli_free( ___VALUE_STR(command) );
                            // use new string
                            ___VALUE_STR(command) = c;
                            // concat grep string
                            if ( mrdc_icli_str_concat(___VALUE_STR(command), ___GREP_CMD_STR) == NULL ) {
                    ...
                    if ( g_current_cmd->mode_info->b_mode_var ) {
                        g_current_cmd->mode_word_cnt = mrdc_icli_word_count( (char *)(g_current_cmd->mode_info->cmd) );
...
)
{
    ...
    r = mrdc_icli_str_sub(_CONST_STR_PREFIX, value_str, 1, NULL);
    switch ( r ) {
    case 1:
        // skip space
        ICLI_SPACE_SKIP(s, &(value_str[mrdc_icli_str_len(_CONST_STR_PREFIX)]));
        for ( tv = g_str_def; tv != NULL; ___NEXT(tv) ) {
            if ( mrdc_icli_str_cmp(s, tv->tag) == 0 ) {
                return 1;
            }
        }
        return -1;
...
#define ___STR_LEN(t)       mrdc_icli_str_len(t->value_str)
    ...
    for ( i=0, t=g_current_cmd->help; i<n; ++i ) {\
        if ( g_current_cmd->grep && i >= (n - ___GREP_WORD_CNT) ) {\
            ...
            if ( t == NULL ) {\
                ...
                switch( r ) {\
                    case 1:\
                        ICLI_SPACE_SKIP(cs, &(t->value_str[mrdc_icli_str_len(_CONST_STR_PREFIX)]));\
                        fprintf(g_fd, "    %s%s\n", cs, (i<n-1)?",":"");\
                        break;\
                    case 0:\
                        fprintf(g_fd, "    \"%s\"%s\n", t->value_str, (i<n-1)?",":"");\
    ...
    for ( c = cmd, not_over = TRUE; not_over; ++c ) {
        ...
        case ICLI_WORD_STATE_VARIABLE:
            ...
            if ( ICLI_IS_(VARIABLE_END, *c) ) {
                ...
                //get variable type
                memset(range, 0, sizeof(icli_range_t));
                rc = mrdc_icli_variable_type_get( cmd_word[j], cmd_type + j, range );
                switch ( rc ) {
                case ICLI_RC_OK:
                    if ( mrdc_icli_variable_data_has_range_get(cmd_type[j]) ) {
...
#define ___GET_ALL(_what_) \
    ...
    ___GET_##_what_( ICLI_VARIABLE_LOOPBACK_ID,            u_loopback_id       ); \
...
)
{
    ...
    r = mrdc_icli_str_sub(_CONST_STR_PREFIX, t->value_str, 1, NULL);
    switch ( r ) {
    case 1:
        // skip space
        ICLI_SPACE_SKIP(s, &(t->value_str[mrdc_icli_str_len(_CONST_STR_PREFIX)]));
        for ( tv = g_str_def; tv != NULL; ___NEXT(tv) ) {
            if ( mrdc_icli_str_cmp(s, tv->tag) == 0 ) {
                return tv->value_str;
            }
...
)
{
    ...
    for ( ++c; ICLI_NOT_(EOS, *c); ++c ) {
        if ( ICLI_IS_(COMMA, *c) ) {
            ...
            memset(s, 0, len + 1);
            (void)mrdc_icli_str_ncpy(s, start, len);

            if ( mrdc_icli_str_to_int(s, &val) == ICLI_RC_OK ) {
                if ( b_deli && val <= min ) {
                    __U_E("invalid range, %d-%d\n", min, val);
                    return FALSE;
                }
                (void)mrdc_icli_str_ncpy(w, s, len);
                w += len;
            } else {
...
static i32 _generate_C_and_H( void )
{
    ...
    if ( g_generate_path ) {
        len = mrdc_icli_str_len(g_generate_path) + mrdc_icli_str_len(g_script_file) + 16;
        ...
        len = mrdc_icli_str_len(g_script_file) + 16;
    ...
    */
    ...
    for ( g_current_cmd = g_command, k = 0; g_current_cmd != NULL; ___NEXT(g_current_cmd), ++k ) {
        ...
        for ( i = 0, t = g_current_cmd->runtime; i < n; ++i ) {
            if ( t && mrdc_icli_str_len(t->value_str) ) {
                ___LINE_GEN( t );
        ...
        for ( i = 0; i < n; ++i ) {
            b_range = FALSE;
            if ( mrdc_icli_variable_data_has_range_get(g_variable_type[k][i]) ) {
                b_range = TRUE;
                sprintf(range_str, "&%s_range_%d", func_name, i);
            }
            fprintf(g_fd, cmd_node_string,
                    i,
                    mrdc_icli_variable_data_name_get(g_variable_type[k][i]),
    ...
    for ( g_current_cmd = g_command, k = 0; g_current_cmd != NULL; ___NEXT(g_current_cmd), ++k ) {
        ...
        for ( i = 0, t = g_current_cmd->mode_var; i < mode_word_cnt && t != NULL; ++i ) {
            if ( g_mode_var_type[k][i] != ICLI_VARIABLE_KEYWORD  &&
                ...
                for ( ; t != NULL && mrdc_icli_str_len(t->value_str) == 0; ___NEXT(t) ) {
                    ;
                }
                if ( t == NULL ) {
                    break;
                ...
                default :
                    fprintf(g_fd, "    %-21s %s = %s;\n",
                            mrdc_icli_variable_data_decl_type_get(g_mode_var_type[k][i]),
                            t->value_str,
                            mrdc_icli_variable_data_init_val_get(g_mode_var_type[k][i]));
                    if ( mrdc_icli_variable_data_pointer_type_get(g_mode_var_type[k][i]) ) {
                        b_pointer_var = TRUE;
                    }
                    break;
                }
        ...
        for ( i = 0, t = g_current_cmd->cmd_var; i < cmd_word_cnt && t != NULL; ++i, ___NEXT(t) ) {
            if ( mrdc_icli_str_len(t->value_str) > 0 ) {
                if ( cmd_var_cnt == 0 ) {
                ...
                case ICLI_VARIABLE_DSCP:
                    if ( b_dscp == FALSE ) {
                        ...
                        if (g_variable_loop_number[k][i]) {
                            fprintf(g_fd, "    %-21s %s[%s];\n",
                                    mrdc_icli_variable_data_decl_type_get(g_variable_type[k][i]),
                                    t->value_str,
                            ...
                            fprintf(g_fd, "    %-21s %s = %s;\n",
                                    mrdc_icli_variable_data_decl_type_get(g_variable_type[k][i]),
                                    t->value_str,
                                    mrdc_icli_variable_data_init_val_get(g_variable_type[k][i]));
                        }
#else
                        fprintf(g_fd, "    %-21s %s = %s;\n",
                                mrdc_icli_variable_data_decl_type_get(g_variable_type[k][i]),
...
#endif
                    b_dscp = FALSE;
                    if ( mrdc_icli_variable_data_pointer_type_get(g_variable_type[k][i]) ) {
                        b_pointer_var = TRUE;
                    }
                    break;
                }
        ...
        if ( b_loop ) {
            for ( i = 0, t = g_current_cmd->cmd_var; i < cmd_word_cnt && t != NULL; ++i, ___NEXT(t) ) {
                if ( mrdc_icli_str_len(t->value_str) > 0 && g_variable_loop_number[k][i] ) {
    ...
    */
    ...
    // #ifndef
    mrdc_icli_to_upper_case(g_script_file);
    fprintf(g_fd, "#ifndef ___%s_ICLI_H___\n", g_script_file);
    fprintf(g_fd, "#define ___%s_ICLI_H___\n\n", g_script_file);
    mrdc_icli_to_lower_case(g_script_file);
...
#define ___HTML_ADD_TO_TAIL(p)\
    (void)mrdc_icli_str_cpy(g_value_str, g_line_buf);\
...
static i32 _html_read( void )
{
    ...
    while (_line_read() == ICLI_RC_OK) {
        /* get g_command_cnt */
        if ( mrdc_icli_str_cmp(g_line_buf, _HTML_COMMAND_CNT) == 0 ) {
            if (_line_read() != ICLI_RC_OK) {
        ...
        /* get g_html_reflink */
        if ( mrdc_icli_str_cmp(g_line_buf, _HTML_REFLINK_BEGIN) == 0 ) {
            ...
            while (_line_read() == ICLI_RC_OK) {
                ___HTML_ADD_TO_TAIL(h->begin);
                if ( mrdc_icli_str_cmp(g_line_buf, _HTML_REFLINK_END) == 0 ) {
        ...
        /* get g_html_command */
        if ( mrdc_icli_str_cmp(g_line_buf, _HTML_COMMAND_BEGIN) == 0 ) {
            h = (_html_element *)icli_malloc(sizeof(_html_element));
            if ( h == NULL ) {
                T_E("icli_malloc\n");
                return ICLI_RC_ERROR;
            ...
            while (_line_read() == ICLI_RC_OK) {
                ___HTML_ADD_TO_TAIL(h->begin);
                if ( mrdc_icli_str_cmp(g_line_buf, _HTML_COMMAND_END) == 0 ) {
                    for (t = h->begin; t->next != NULL; ___NEXT(t)) {
                        ;
                    }
                    h->end = t;
                    break;
                } else if ( mrdc_icli_str_sub("<a name=", g_line_buf, 1, NULL) == 1 ) {
                    //read command
                    if ( _line_read() != ICLI_RC_OK ) {
                        T_E("read command\n");
                        return ICLI_RC_ERROR;
...
#define ___HTML_TAG_BEGIN_ADD(s)\
    ___t_ALLOC(t, ___HTML_DEF_TAG_SIZE);\
    (void)mrdc_icli_str_cpy(t->value_str, s);\
    h->begin = t;\
    p        = t;
...
)
{
    ...
    c = g_line_buf;
    ___HTML_STRING_ADD(mrdc_icli_str_len(c) + 4, "%s", c);
    h->command = t;
    h->cmd_ptr = cmd;
...
)
{
    ...
    for ( x = cmd->doc_cmd_desc; x != NULL; ___NEXT(x)) {
        if ( mrdc_icli_str_len(x->value_str) ) {
    ...
    memset(w, 0, len);
#if 1 /* CP, 09/30/2013 16:44, Bugzilla#12878 - ICLI presents symbolic constants from COMMANDs to the user */
    (void)mrdc_icli_str_cpy(w, cmd->var_cmd);
#else
    (void)mrdc_icli_str_cpy(w, cmd->command->value_str);
#endif
    for ( i = 0, x = cmd->help; i < cmd->command_word_cnt; ++i ) {
        c = mrdc_icli_word_get_next(&w);
        if ( c == NULL ) {
            T_E("COMMAND = %s\n", cmd->command->value_str);
            T_E("get word in %s\n", c);
            return NULL;
        ...
        if ( x && mrdc_icli_str_len(_help_str_get(x)) ) {
    ...
    case ICLI_PRIVILEGE_MAX:
        ___HTML_STRING_ADD(mrdc_icli_str_len(cmd->privilege->value_str) + 4, "%s", cmd->privilege->value_str);
        break;
...
#define ___HTML_ELEMENT_INSERT(element)\
    ...
    if ( e ) {\
        for ( p = NULL; e != NULL; p = e, ___NEXT(e)) {\
            if ( mrdc_icli_str_cmp(e->command->value_str, h->command->value_str) == 1 ) {\
                break;\
            }\
        }\
        if ( e ) {\
...
static BOOL _html_cmd_ref_add( void )
{
    ...
    for (g_current_cmd = g_command; g_current_cmd != NULL; ___NEXT(g_current_cmd)) {
        ...
        if ( g_current_cmd->property &&
             mrdc_icli_str_str("ICLI_CMD_PROP_DISABLE", g_current_cmd->property->value_str) != NULL ) {
            continue;
        }

        //ICLI_CMD_PROP_INVISIBLE
        if ( g_current_cmd->property &&
             mrdc_icli_str_str("ICLI_CMD_PROP_INVISIBLE", g_current_cmd->property->value_str) != NULL ) {
            continue;
        }
        ...
        if ( g_current_cmd->grep ) {
            c[mrdc_icli_str_len(c) - mrdc_icli_str_len(___GREP_CMD_STR)] = 0;
            g_current_cmd->command_word_cnt = mrdc_icli_word_count( c );
            if ( g_current_cmd->command_word_cnt < 0 ) {
                T_E("COMMAND = %s\n", ___VALUE_STR(command));
                T_E("fail to get command word count\n");
                return FALSE;
...
static i32 _generate_HTM( void )
{
    ...
    // get HTML file name
    if ( mrdc_icli_str_len(g_generate_path) ) {
        len = mrdc_icli_str_len(g_generate_path) + mrdc_icli_str_len(g_script_file) + 16;
...
)
{
    ...
    c = script_path + (mrdc_icli_str_len(script_path) - mrdc_icli_str_len(_SCRIPT_SUFFIX));
    if ( mrdc_icli_str_cmp(c + (mrdc_icli_str_len(c) - mrdc_icli_str_len(_SCRIPT_SUFFIX)), _SCRIPT_SUFFIX) != 0 ) {
    ...
    g_script_path = icli_malloc(mrdc_icli_str_len(c) + 1);
    if ( g_script_path == NULL ) {
        T_E("fail to allocate memory for g_script_path\n");
        return ICLI_RC_ERROR;
    }
    (void)mrdc_icli_str_cpy(g_script_path, c);

    f = *p;
    *p = 0;

    g_script_file = icli_malloc(mrdc_icli_str_len(c) + 1);
    if ( g_script_file == NULL ) {
        T_E("fail to allocate memory for g_script_file\n");
        return ICLI_RC_ERROR;
    }
    (void)mrdc_icli_str_cpy(g_script_file, c);
...
)
{
    ...
    while (_line_read() == ICLI_RC_OK) {
        ...
        */
        ...
        // must #define
        if ( mrdc_icli_str_sub("#define", c, 0, NULL) != 1 ) {
            continue;
        }
        // skip #define
        c += 7;
        ...
        *e = ICLI_EOS;
        ...
        (void)mrdc_icli_str_cpy(g_tag_str, c);
...
)
{
    ...
    srand( icli_os_current_time_get() + u + mrdc_icli_str_len(script_file) );
...
#define ___LINE_OPT(i) \
    (mrdc_icli_str_cmp(argv[i], "-L") == 0 || mrdc_icli_str_cmp(argv[i], "-l") == 0)

#define ___GENERATE_OPT(i) \
    (mrdc_icli_str_cmp(argv[i], "-G") == 0 || mrdc_icli_str_cmp(argv[i], "-g") == 0)

#define ___INCLUDE_OPT(i) \
    (mrdc_icli_str_cmp(argv[i], "-I") == 0 || mrdc_icli_str_cmp(argv[i], "-i") == 0)
...
int main(i32 argc, char *argv[])
{
    ...
    if ( generate_path ) {
        // get length
        r = mrdc_icli_str_len( generate_path );
        ...
        (void)mrdc_icli_str_cpy(g_generate_path, generate_path);
