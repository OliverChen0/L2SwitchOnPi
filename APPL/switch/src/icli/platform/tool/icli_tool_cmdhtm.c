...
)
{
    ...
    while (_line_read(f) == ICLI_RC_OK) {
        /* get g_read_cmd_cnt */
        if ( mrdc_icli_str_cmp(g_line_buf, _HTM_COMMAND_CNT) == 0 ) {
        ...
        /* get g_read_reflink */
        if ( mrdc_icli_str_cmp(g_line_buf, _HTM_REFLINK_BEGIN) == 0 ) {
            ...
            while (_line_read(f) == ICLI_RC_OK) {
                ___HTM_ADD_TO_TAIL(h->begin);
                if ( mrdc_icli_str_cmp(g_line_buf, _HTM_REFLINK_END) == 0 ) {
        ...
        /* get g_read_command */
        if ( mrdc_icli_str_cmp(g_line_buf, _HTM_COMMAND_BEGIN) == 0 ) {
            h = (_htm_element_t *)icli_malloc(sizeof(_htm_element_t));
            if ( h == NULL ) {
                __T_E("icli_malloc\n");
                fclose( f );
            ...
            while (_line_read(f) == ICLI_RC_OK) {
                ___HTM_ADD_TO_TAIL(h->begin);
                if ( mrdc_icli_str_cmp(g_line_buf, _HTM_COMMAND_END) == 0 ) {
                    for (t = h->begin; t->next != NULL; ___NEXT(t)) {
                        ;
                    }
                    h->end = t;
...
)
{
    ...
    while ( h ) {
        ...
        for ( t = h->begin; t; t = t->next ) {
            if ( mrdc_icli_str_sub("<a href=", t->value_str, 1, NULL) == 1 ) {
                break;
            }
        }
        if ( t == NULL ) {
    ...
    while ( h ) {
        ...
        // find <a name
        for ( t = h->begin; t; t = t->next ) {
            if ( mrdc_icli_str_sub("<a name=", t->value_str, 1, NULL) == 1 ) {
                break;
            }
        }
        if ( t == NULL ) {
...
)
{
    ...
    for ( i = 2; i < argc; ++i ) {
        ...
        ++j;
        // copy to g_value_str
        (void)mrdc_icli_str_cpy(g_value_str, &(argv[i][j]));
    ...
    g_htm_file = icli_malloc(mrdc_icli_str_len(argv[1]) + 32);
    if ( g_htm_file == NULL ) {
